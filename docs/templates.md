...
container definition: too long to read inline in `infra.yml`, and often something you want
...
it does in `infra.yml` — including resource references and their dependency edges:
...
there is no templating pass over `infra.yml` itself, and there is not going to be one.
