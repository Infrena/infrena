...
Removing an environment from `infra.yml` does not silently orphan what it held. Infrena
...
Declarations are typed, and live in `infra.yml`:
...
`infra.yml`; the two are different documents and are decoded differently.
