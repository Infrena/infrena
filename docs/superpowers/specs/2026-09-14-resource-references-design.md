...
**THE PLUGIN IS THE DECIDER. Core never guesses.** Infrena cannot know that Cloud
Control's `AWS::EC2::Subnet.VpcId` wants a VPC's id rather than its arn — that is knowledge
about an API, and it belongs to whoever owns the API. So the fact lives in the plugin's
schema, is generated by the plugin from its own source of truth, and core does nothing with
it but read it, exactly as it already reads `Computed`, `Sensitive`, `ForceNew` and
`Aliases`. §4 is the AWS plugin's problem of populating its own schema; it is in this spec
only because the engine's guarantees depend on that population being honest, and nothing in
it is engine logic.
...
## 4. Where the table comes from — PLUGIN-SIDE, not engine logic

Everything in this section happens inside `infrena-provider-aws`, at generation time,
before a schema is ever handed to the engine. It is here because §2.2's degradation
promise and §5's type check are only as good as the declarations behind them, so a spec
that described the engine's half and stayed silent on how the other half gets populated
would be describing a guarantee it could not keep. A different plugin for a different cloud
solves this its own way, or declares nothing and loses only the sugar.
...
- **Passing the whole resource to the plugin and letting it project at runtime.**
  Considered seriously, and rejected on three specific costs rather than on principle. The
  principle behind it is RIGHT and this spec adopts it: the plugin decides. What is rejected
  is deciding at Create/Update time rather than in the schema.
  - **The plan stops telling the truth.** `internal/planner/planner.go:263` resolves a
    reference at PLAN time whenever the target already exists and its attribute is known,
    so `vpc_id: ${vpc}` against an existing VPC renders the real `vpc-0abc…` today. If the
    plugin projects during Create, the engine has no value when it renders, and a perfectly
    knowable id shows as a map or as "known after apply". The plan is the product.
  - **The type check moves to apply.** `vpc_id: ${database}` becomes an API rejection
    mid-run, after other resources exist, instead of a compile error before anything runs —
    losing the half of §1's value that is worth more than the projection.
  - **A projected resource is indistinguishable from a map.** `DesiredResource.Attrs` is
    `map[string]value.Value` and a whole resource is a `KindMap`, so an attribute that
    legitimately takes a map — tags — and one holding a projected resource look identical
    without a new Kind or a wrapper. Declaring instead means NO whole-resource value ever
    reaches a plugin, and the problem does not arise.

  The variant that would keep the first two — a `project(type, attribute, resourceAttrs)`
  protocol method called at plan time as well as apply — costs an RPC per projected
  attribute per plan, makes a plugin a participant in planning, and still needs a declared
  type for the compile-time check, so it carries the schema data AS WELL AS the call.
