...
// azDefinition is a resource type with an optional+computed attribute: one configuration
...
//
// A planner that reads a returned value configuration does not set as "removed from
// configuration" proposes to unset it. The cloud picks an availability zone, infrena
// proposes unsetting it, the apply succeeds, the cloud picks again — and the project
// never converges. That breaks the no-op plan for every attribute of this shape, of
...
//
// Both rest on one cause: without schema access the renderer can neither map a
// canonical name to its friendly alias nor tell a provider-chosen value from a
// configured one. One change covers both, and `Display` is dead code without it.
...
// Render is public and pure; a nil lookup must fall back to canonical names with no
// notes.
