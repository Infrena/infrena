...
func deferred(t *testing.T, src string) value.Value {
	t.Helper()
	expr, ds := expressions.Parse(src, value.Origin{File: "infrena.yml", Line: 1})
	if ds.HasErrors() {
...
// planner diffs configuration's DependsOn against, so a fixture recording a
// resource with edges in configuration and none in state describes a state
// production no longer produces, and reports an update on every run — which
// reads as a convergence bug when the fixture is what is broken. Seed the
// precondition the way production establishes it, or the test measures the
// author's assumptions instead of the code.
...
// TestPlanConvergesWhenAReferencedAttributeIsAlreadyKnown: desired == actual,
// but one resource's configuration references another's computed attribute.
//
// Unless the planner finishes those references, the diff compares the recorded
// "net-1" against a desired value that was unknown at compile time and stays
// unknown forever, so the plan proposes an update — and so does the plan after
...
func TestPlanConvergesWhenAReferencedAttributeIsAlreadyKnown(t *testing.T) {
	...
	for i := range 50 {
		...
		if p.HasChanges() {
			t.Fatalf("run %d: desired equals actual, but the plan proposes %d change(s): %+v",
...
// TestPlanConvergesThroughAChainOfReferences extends the same rule past one
// hop: application refers to database, which refers to network. A resolution
// pass that only looked one level deep, or that ordered resources by anything
// other than their dependencies, converges the first hop and leaves the second
...
func TestPlanConvergesThroughAChainOfReferences(t *testing.T) {
	...
	if p.HasChanges() {
		t.Fatalf("desired equals actual across a reference chain, but the plan proposes changes: %+v", p.Operations)
	}
...
func TestPlanIsUnchangedForAConfigurationThatWasNeverApplied(t *testing.T) {
	...
	// Identical inputs, identical plan, repeatedly, so that a resolution order
	// derived from map iteration would show up rather than passing by luck.
	for i := range 50 {
		p, ds := Compute(cfg, nil, nil, planOpts(t))
		if ds.HasErrors() {
			t.Fatalf("run %d: unexpected errors:\n%s", i, rendered(t, ds))
		...
		if string(got) != string(want) {
			t.Fatalf("run %d: plan differs; identical inputs must produce an identical plan:\n--- want ---\n%s\n--- got ---\n%s", i, want, got)
		}
	}
}

// TestPlanIsDeterministicWhenReferencesResolve is the same rule on the applied
// path, where resolution actually does work: fifty plans over identical
// configuration, state and observations must be byte-identical.
...
// which is what resolutionOrder deliberately returns for a malformed graph —
// produces a visibly different sequence.
...
// TestResolutionOrderIncludesAddressesOnlyInState makes sure the ordering does
// not quietly drop the resources a plan proposes destroying: they appear in
// state and not in configuration.
...
//
// Compute decides operations in dependency order and emits them in address
// order. Emitting in resolution order instead passes every test of the deciding
// order, even though the plan artifact's byte-stability rests on emission
// order, and even though a reader who meets resolutionOrder first could
// reasonably take it for THE order.
//
...
// because it sorts first. A fixture whose dependency order agreed with its
// alphabetical order would pass either way.
func TestOperationsAreEmittedInAddressOrder(t *testing.T) {
	...
	for i := range 50 {
		...
		if !equalStrings(got, wantEmitted) {
			t.Fatalf("run %d: operations emitted as %v, want %v (sorted by canonical address)",
				i, got, wantEmitted)
		}
	}
}
...
// a resource that sorts ahead of it. Which order a plan reports its problems in
// is part of plan determinism too.
//
...
// happened to contain a cycle would never be destroyed: a removal lost to a
...
func TestResolutionOrderFallsBackWithoutLosingRemovals(t *testing.T) {
	...
	if got := operation(t, p, "mu").Kind; got != OpForget && got != OpDestroy {
		t.Errorf("mu: kind %v, want a removal — it is in state and not in configuration", got)
	}
}
