...
// decoded without it applies correctly itself and silently breaks the next plan's
// destroy ordering.
...
//
// Making UnmarshalText fall back to OpNoOp is the worst available outcome: a create
// from a newer build decodes as "no operation", the apply reports success, and nothing
// is created. A plan that silently does less than it says is worse than one that
// refuses.
//
