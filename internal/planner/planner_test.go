...
// --- one test per row of the plan decision table ---------------------------
...
func TestInConfigNotInStateIsCreate(t *testing.T) {
	...
	if op.Before != nil {
		t.Error("Before must be nil for a create")
	}
...
func TestNoDifferencesIsNoOp(t *testing.T) {
	...
	if p.HasChanges() {
		t.Error("desired equals actual, so the plan must propose no changes")
	}
}
...
func TestUpdatableDifferenceIsUpdate(t *testing.T) {
	...
	if op.Before == nil || op.After == nil {
		t.Error("an update populates both Before and After")
	}
}
...
func TestObservedAbsentIsRecreate(t *testing.T) {
	// In config, in state, but the provider no longer has it: something deleted
	// it outside infrena.
	cfg := config(configured("net", "fake.network", map[string]value.Value{
		"cidr": str("10.0.0.0/16"),
	}))
	...
	op := only(t, p)
	if op.Kind != OpCreate {
		t.Fatalf("Kind = %s, want create — a resource deleted outside infrena is recreated", op.Kind)
	}
	if op.Before != nil {
...
func TestNotInConfigButPresentIsDestroy(t *testing.T) {
	...
	if op.Kind != OpDestroy {
		t.Fatalf("Kind = %s, want destroy — it is in state and not in configuration", op.Kind)
	}
	if op.After != nil {
		t.Error("After must be nil for a destroy")
	}
...
func TestPlanRecordsItsInputFingerprints(t *testing.T) {
	...
	if p.ConfigHash == "" {
		t.Error("ConfigHash must be populated so a stale plan can be detected")
	}
	if p.StateHash == "" {
...
// hasUnknown directly: a composite Value whose Raw does not match its Kind must
// read as unknown, the same conservative rule value.Equal applies to malformed
// values and for the same reason — a value that cannot be PROVEN fully known
// must not be treated as fully known. Discarding the type assertion's ok is the
// shape of bug that makes two malformed values compare equal.
...
func TestDestroyReportsDependentsFromState(t *testing.T) {
	...
	// the destructive-change warning silently disappears.
	//
...
func TestPlanIsDeterministicAcrossTwentyRuns(t *testing.T) {
	// Identical configuration, state and observations produce an equivalent plan.
	// The clock deliberately advances on every call, so this also proves the
	// canonical form does not depend on CreatedAt.
	var (
	...
	for i := range 20 {
		...
		if !bytes.Equal(want, got) {
			t.Fatalf("run %d produced a different plan; identical inputs must produce byte-identical output\nwant: %s\ngot:  %s", i, want, got)
		}
...
// was reported, because the error is reported either way.
...
// Treating the difference as a change proposes replacing running instances,
// because the attribute holding it forces a new resource.
//
