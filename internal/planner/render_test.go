...
// scopedPlan hand-builds its values, like mixedPlan, rather than routing through
// the compiler, so this golden depends only on the renderer.
		...
		Operations: []Operation{
			{
				...
				After: map[string]value.Value{
					...
					// Won by --var-file. The annotation names the file
					// itself, not the generic "--var" Scope.String() would
					// otherwise give every ScopeCLIOverride value.
...
// TestRenderScopedValuesShowPrecedence is the golden covering a whole plan with
// mixed scopes — defaults annotated [default], and the full precedence ladder —
// in one artifact a person would actually read.
...
// TestRenderNeverLeaksASecretForAnyOperationKind. Sensitivity is per-leaf, so a
// secret nested inside a list or a map must be redacted just as surely as one at
// the top level — for every operation kind and under both RenderOptions.
...
// file cannot: values whose Kind and Raw disagree, or whose Kind was never set.
// It drives renderAnnotated, so the annotation path is covered too — a malformed
// value must not leak through that either. See value.Format's doc comment.
...
// directly. Annotate has its own tests; what this pins is the WIRING — that the
// plan renderer actually reaches Annotate with the plan's own FormatOptions,
// rather than keeping a planner-local copy. A test that called value.Annotate
// would pass even against a renderer that never delegates to it.
//
...
// TestRenderAnnotatesCLIOverrideWithSuppliedByOnly: at ScopeCLIOverride,
// renderAnnotated names the value's own SuppliedBy in preference to the scope's
// generic "--var" label, and nowhere else.
//
// SuppliedBy is a dedicated field rather than a reuse of Origin because
// eval.go's OpVarRef case re-origins every "${var.x}" reference to the
// referencing expression's site, so Origin does not survive to the renderer for
// any value that reaches a resource attribute the normal way. This test
// constructs Values directly and so cannot see whether that survival holds
// through real evaluation; internal/cli covers that end to end. What it proves
// is the renderer's wiring and the scope restriction (last case).
...
// defence in depth against a Plan built by some other producer — one read back
// from disk, or a second planner. Render's documented contract is that
// identical plans render identical text, and the input here is the adversarial
// one that contract has to survive, so the fixture hands it reasons in an order
// Compute would never produce.
//
...
// TestRenderIsDeterministicAcrossRepeatedCalls pins the plan-text half of plan
// determinism: the same plan must render the same bytes every time. Render walks
// Before and After as maps, so the ordering comes from unionKeys and nothing
// else; without it Go's randomised range order leaks into the artifact a user
// reads before approving a change.
//
...
// insertion order, so a one-shot comparison passes most of the time even when
// the sort is gone. Five attributes give enough orderings that 30 renders never
// all agree by accident.
//
// Two of the After attributes carry a Scope, so this also covers value.Annotate's
// output being as deterministic as value.Format's.
	...
	plan := func() *Plan {
		return &Plan{
			...
			Operations: []Operation{{
				...
				// Already in canonical order, so renderForcedBy's own sort is
				// a no-op here: this test is about the attribute maps, and the
				// reason ordering has its own test. Keeping them separate is
				// what lets each name which decision it pins.
				Reasons: []ChangeReason{
					{Attribute: "engine", ForceNew: true},
...
// TestRenderNotesAResourceThatMayHaveMovedBetweenModules is a cost of module
// flattening, rendered where a user meets it.
//
...
//
// This duplicates the integration suite's lineContaining because a test helper
// does not cross packages.
...
// TestRenderCreateSaysWhyAResourceIsBeingRecreated covers the difference between
// the two creates the planner produces: one for a resource that has never
// existed, and one for a resource that IS in state but that the provider no
// longer reports (planner.go, operationFor). Both render as `+`, so unless
// renderMetadataLines runs for creates too, the Reasons the planner attaches are
// dropped and the two look identical.
//
// That silence is the whole cost. A user who has just imported infrastructure
// and sees `+` beside a resource their state file plainly contains has no way to
// tell drift from a plan that has lost track of it, and the plan gives them
// nothing to look up.
func TestRenderCreateSaysWhyAResourceIsBeingRecreated(t *testing.T) {
	p := &Plan{
		Project:     "myapp",
		Environment: "dev",
		Operations: []Operation{
			{
				Address: address.Address{Name: "fresh"}, Type: "fake.network", Kind: OpCreate,
				After: map[string]value.Value{"cidr": value.String("10.0.0.0/16", value.SourceExplicit)},
			},
			{
				Address: address.Address{Name: "vanished"}, Type: "fake.network", Kind: OpCreate,
				After:   map[string]value.Value{"cidr": value.String("10.1.0.0/16", value.SourceExplicit)},
	...
	if !strings.Contains(got, "the provider no longer reports this resource; it will be recreated") {
		t.Errorf("a create carrying a reason must say why it is happening:\n%s", got)
	}

	// The note belongs to the resource that carries it. A reason rendered
	// under every create would be worse than none: it would teach the reader
	// to ignore the line.
	fresh := strings.Index(got, "fake.network.fresh")
	vanished := strings.Index(got, "fake.network.vanished")
	note := strings.Index(got, "no longer reports")
	if fresh < 0 || vanished < 0 || note < 0 {
		t.Fatalf("expected both creates and the note to render:\n%s", got)
	}
	if note < vanished {
		t.Errorf("the note rendered under the wrong resource:\n%s", got)
	}
}
