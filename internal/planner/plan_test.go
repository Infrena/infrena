...
func TestCanonicalExcludesCreatedAt(t *testing.T) {
	// Identical inputs produce a byte-identical plan. CreatedAt records when the
	// plan was made, which is not one of its inputs, so the canonical form omits
	// it.
...
// place it can actually regress. The artifact does not call OpKind.String() —
// it relies on encoding/json finding MarshalText. Delete MarshalText and every
...
func TestUnknownValuesSurviveIntoTheArtifact(t *testing.T) {
	// After may contain unknowns. They must round-trip as "not yet known", never
	// as a zero value.
	p := &Plan{
		Version: PlanVersion,
		Operations: []Operation{{
...
func TestSensitiveValuesArePresentInTheArtifact(t *testing.T) {
	// Deliberate, and load-bearing: the saved plan contains sensitive values
	// because apply needs them, which is why the CLI writes it mode 0600.
	// Redaction belongs to Render, not here. Do not "fix" this test by redacting
	// the artifact — applying a saved plan would then apply the wrong values.
	p := &Plan{
		Version: PlanVersion,
		Operations: []Operation{{
			Address: addr("db"), Type: "fake.database", Kind: OpCreate,
...
func TestDiagnosticsAreCarriedInTheArtifact(t *testing.T) {
	// A plan whose diagnostics contain an error is never applyable, so the
	// severity has to survive serialisation legibly.
	p := &Plan{
		Version: PlanVersion,
		Diagnostics: []diag.Diagnostic{{
			Severity: diag.SeverityError,
			Summary:  "database is protected by prevent_destroy",
			Origin:   value.Origin{File: "infrena.yml", Line: 12, Column: 3},
	...
	for _, want := range []string{`"severity":"Error"`, "prevent_destroy", "infrena.yml:12:3", `"database"`} {
...
func TestDiagnosticsRelatedAreSortedInTheCanonicalForm(t *testing.T) {
	...
	// address order. The planner builds diagnostics for dependency relationships,
	// so this is a real map-to-slice-shaped boundary, the same as Dependents a
	// few lines above in encode. Three entries, not two: with two, a wrong
	// implementation has a 50% chance of looking right.
	p := &Plan{
		Version: PlanVersion,
		Diagnostics: []diag.Diagnostic{{
			Severity: diag.SeverityError,
...
func TestDependentsAreCarriedAndSortedInTheCanonicalForm(t *testing.T) {
	// The count is what lets a warning about a destructive change be rendered,
	// and Render sees nothing but the plan. Encoding sorts, so two plans over
	// identical inputs agree on the field whatever order they assembled it in.
	forward := &Plan{
		Version: PlanVersion,
		Operations: []Operation{{
...
func TestHasChangesIgnoresNoOpsOnly(t *testing.T) {
	...
	if quiet.HasChanges() {
		t.Error("a plan of nothing but NoOps has no changes")
	}

	// Forget counts. The provider is never called, but state changes, and
	// `infrena plan` must exit ExitChanges so CI notices.
...
//
// The artifact is a documented, versioned format that `--output` writes for other
// programs to read. Without this, a field can be added, renamed or dropped from
// planWire or operationWire with nothing to notice.
//
...
func TestTheArtifactsKeysAreFrozen(t *testing.T) {
	...
	// INSIDE `lifecycle` too. A nested object in a versioned format is still the format:
	// treating it as one opaque key lets a field be added to it with nothing noticing.
...
// the instance cannot be applied to the right account.
...
// TestDecodePlanRefusesAReportLine. json.Decoder reads one value and stops, so
// pointed at a report stream DecodePlan would consume the meta line. Unknown
// fields are ignored and report.Version is 1, the same integer PlanVersion
// checks, so it would succeed and yield a plan with no operations that applies
// nothing, silently.
