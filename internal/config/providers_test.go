...
// Decoding `providers:`. No resolution, no registry, no dispatch.
...
// TestTwoInstancesWithTheSameNameIsAnError, in both spellings the collision
// takes.
func TestTwoInstancesWithTheSameNameIsAnError(t *testing.T) {
	for _, tc := range []struct{ label, body string }{
		...
		t.Run(tc.label, func(t *testing.T) {
			...
			// rendering, because a position has two spellings: describeOrigin
			// writes "line 3 of <file>" while the Origin itself renders
			// "<file>:4:5".
...
// TestConfigAndDefaultsAreSeparate is what the nesting exists for. BOTH
...
// which is exactly the collision this block must refuse — a shape that cannot
// express it would refuse it SILENTLY.
...
// A resource names its provider INSTANCE.
...
//
// A resources file already refuses foreign keys, so twice in the project file is the
// only reachable case.
