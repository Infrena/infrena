...
// TestDecodeModuleFile is the happy path.
//
...
// TestModuleFileRejectsProjectLevelKeysAsUnknownKeys. The module decoder has no
// case for `project`, `environments` or `variables`, so each falls out of the
...
// TestModuleFileAcceptsNestedModules. Recursion is bounded at 32 instantiations
// rather than forbidden, and a module may declare `modules:` of its own, decoded
// by the same decoder rather than a second one. Without this, depth is always 1
// and the depth bound and cycle detector have nothing to detect.
//
...
//
// `${...}` is this language's only reference syntax, with `$${` as its escape, so a bare scalar is a literal everywhere else. Decoded as
...
func TestBareOutputReferenceIsRefused(t *testing.T) {
	...
	// dotted. A guard accepting ANY number of identifier segments refuses
	// `db.example.com` with an error telling the user they forgot `${...}` when
	// they had not.
...
func TestLoadModuleDiagnoses(t *testing.T) {
	...
	t.Run("infrena.yml where module.yml was expected", func(t *testing.T) {
		dir := t.TempDir()
		...
		if err == nil {
			t.Fatal("want an error when the directory holds infrena.yml instead")
		}
