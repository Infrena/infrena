...
// TestDecodeEnvironmentsBlock decodes the FLAT override spelling.
...
// TestBothOverrideSpellingsAreAccepted: overrides may be nested under
// `variables:` or written as flat keys. Both are documented, so both must work.
...
// TestEnvironmentFileMergesWithTheBlock is the accepting direction: `extends`
// belongs in the block and overrides belong in the file, so a user doing both
...
// TestOverrideSetInTwoPlacesIsRejected is the refusing direction, in all three
// shapes a collision can take.
...
// TestStageTwoDoesNotJudgeExtendsTargets. Cycles and unknown parents need the
// whole set of environments, which is stage 3's job. Two implementations of one
// concept drift apart silently.
...
// broken code.
//
// Sorting here is what spares every consumer a re-sort whose only job would be
// undoing map iteration.
