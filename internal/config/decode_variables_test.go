...
// is a precondition PRODUCTION establishes, and bugs hide behind fixtures that
// manufacture preconditions production never establishes.
...
// TestDecodeVariableDeclaration decodes a complete declaration.
...
func TestUnknownVariableTypeIsRejected(t *testing.T) {
	...
	// available: say what was expected and what to do.
...
// TestUnusableTypeSuppressesTheBoundDiagnostic covers two gaps with one
...
// TestBoundIsCoercedToTheDeclaredType is the rule later stages rely on: after
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if v.Min.Kind != v.Type {
				t.Errorf("bound Kind %v != declared Type %v; the bound must be coerced to the declared kind", v.Min.Kind, v.Type)
			}
...
// TestDefaultIsCoercedToTheDeclaredTypeRegardlessOfKeyOrder pins that a
// numeric `default:` coerces to its variable's declared type
...
// TestDefaultThatCannotBeCoercedIsRejected is the refusing
...
// TestDefaultThatOverflowsFloatIsRejected is the counterpart to
...
// TestUntypedDefaultIsNotCoerced pins the boundary: an untyped
...
// resolved before any expression scope exists, so an unresolved
...
// The sibling check on infrena.yml's `default:` is covered elsewhere; this is
...
// TestVariableBodyMustBeAMapping, and the Action must tell the user where a
// bare value actually goes.
//
...
// package, so it is the fragment that actually distinguishes "the body is the wrong shape" from "the body
// says nothing".
...
// provenance. decodeValue marks everything SourceExplicit because
// that is what an attribute in infrena.yml is; a value from variables.yml is a
...
// scalar leaves cannot tell a genuinely recursive retagSource from
// `v.WithSource(src)` called once at the top. `ports`
...
// TestEmptyOptionalFilesAreNotErrors, in both directions: an empty
// variables.yml is fine, an empty infrena.yml is not.
func TestEmptyOptionalFilesAreNotErrors(t *testing.T) {
	...
	if !ds.HasErrors() {
		t.Fatal("an empty infrena.yml must still be an error")
	}
}
