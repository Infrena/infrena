...
`)
	...
	// Provenance is per-leaf, so a composite's children carry it
...
// TestDecodeVariableFileStampsSuppliedByOnEveryLeafAtCLIOverride: a --var-file
// decode (ScopeCLIOverride) stamps every leaf's SuppliedBy with f.Path,
// per-leaf for the same reason Source and Scope are — a composite's children
// must carry it too, or a nested value could not name where it came from any
// better than the composite shell around it.
...
// when a variable file uses one of infrena.yml's own top-level block names —
// almost always a sign the value belongs in infrena.yml, not here.
