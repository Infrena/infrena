...
`)
	...
	if filepath.Base(attr.Origin.File) != "infrena.yml" {
...
`)
	...
	if !attr.HasExpressions {
		t.Error("an attribute containing ${...} must be flagged so the expression parser can find it")
	}
...
`)
	_, ds := Decode(files)
	if len(ds) < 3 {
		t.Errorf("got %d diagnostics, want at least 3 — validation reports every problem in one pass", len(ds))
	}
}

func TestLoadReportsMissingProjectFile(t *testing.T) {
	if _, err := Load(t.TempDir()); err == nil {
		t.Error("a directory with no infrena.yml must be an error naming the missing file")
	}
}

func TestDecodeReportsMalformedYAML(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "infrena.yml"), []byte("project: [unclosed\n"), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
...
//
// Logical names must be unique within a module: stage 7 keys its resources by
// address, so two entries sharing a name leave one silently winning and an
// entire resource vanishing from the plan without a word. Stage 2 is the only
// stage that still holds the line numbers for a diagnostic worth reading.
...
// A value read by its surface text rather than by its type yields a silently
// wrong string — usually empty — with no diagnostic at all.
...
`,
			...
			// "has no `type`" is the confusing downstream message a reader must
			// not be sent to instead of the real problem.
			notWant: "has no `type`",
		},
		{
...
// TestAliasDoesNotBecomeTheAnchorName is the sharpest form of that: read by
// surface text, `engine: *shared` becomes the string "shared" — the anchor's
// name — so a resource gets a plausible-looking wrong value rather than an
// empty one.
...
// TestAnEnvironmentTypeKeyIsRefused, rather than ignored, and that distinction
// is the whole point. Every other key in an environment block becomes a
// variable override, so leaving `type:` alone does not make it inert: it
// silently declares a VARIABLE named `type`.
func TestAnEnvironmentTypeKeyIsRefused(t *testing.T) {
	...
	// older example has nowhere to go.
	for _, want := range []string{"type", "variables", "configuration.md"} {
...
// `skip` and `only` — decoding only. A resource carrying them still appears
// everywhere until stage 5 resolves them.
...
// TestSkipSurvivesInAModuleFile — they are allowed inside a module, so a module
