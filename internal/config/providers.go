...
// providersListHint shows the shape `providers:` takes, for every diagnostic
// that has to correct it. One copy, so the examples cannot drift apart.
const providersListHint = "providers:\n  - plugin: aws\n    name: main      # optional; defaults to the plugin name\n    iam-role: ...   # the plugin's own configuration"

// decodeProviders decodes `providers:`.
//
// It is a list. A mapping is refused rather than accommodated: a YAML map
// cannot hold two `aws` keys, so the map form cannot express two instances of
// one plugin, and it would lose one silently.
...
func decodeProviderEntry(path string, index int, node *yaml.Node, ds *diag.Diagnostics) (ProviderDecl, bool) {
	...
	for i := 0; i+1 < len(node.Content); i += 2 {
		...
		default:
			// Everything else is the plugin's own configuration. Which keys are
			// meaningful is the plugin's business: validating them here would
			// make adding a provider option a change to the core.
	...
	// An unnamed instance is named after its plugin, which is also what keeps a
	// state file written before instances existed correct: it holds the plugin
	// name, which is this instance's name.
	if d.Name == "" {
...
// checkProviderNames refuses two instances resolving to one name. There is no
// precedence to invent: whichever entry won, the other's resources would be
// created in, and destroyed from, an account nobody named.
...
// none: the one marked `default: true`, else the first. Two marked is refused
// for the same reason two names are. It is a separate pass because "is there
// another one" is not answerable until every entry has been read.
