...
// TestLoadOrderIsFixedAndNotTheFilesystemOrder.
//
...
// code:
//
//   - "aaa" and "bbb" sort before "infrena.yml" as full paths
//     ("<dir>/environments/aaa.yml" < "<dir>/infrena.yml"), so a global path
...
// TestLoadTagsEachFileWithItsKind pins that a consumer can tell the three shapes
...
func TestLoadTagsEachFileWithItsKind(t *testing.T) {
	...
	if files[0].Kind != FileProject || files[0].Environment != "" {
		t.Errorf("infrena.yml: Kind=%v Environment=%q", files[0].Kind, files[0].Environment)
	}
...
// TestLoadTreatsTheOptionalFilesAsOptional. Each case must return exactly the
// project file and no error.
...
// TestLoadAcceptsBothYAMLSpellings: .yaml must work, not merely fail loudly
// when doubled.
...
// TestLoadRefusesAnAmbiguousEnvironment is that rule's error half. Picking one
...
// TestLoadReportsMalformedOptionalFiles, for both optional shapes.
...
// TestLoadStillDemandsTheProjectFile. Making three files optional must not make
// the fourth one optional too: an empty dir returning zero files and no error
// would compile a config with no resources, which reads as "everything was
// removed" — a plan destroying every managed resource.
func TestLoadStillDemandsTheProjectFile(t *testing.T) {
	...
	_, err := Load(dir)
	if err == nil {
		t.Fatal("Load accepted a directory with no infrena.yml")
	}
	if !strings.Contains(err.Error(), "infrena init") {
		t.Errorf("error should still suggest `infrena init`: %v", err)
	}
}
...
// TestLoadReportsEnvironmentsAsAPlainFile covers environments/ existing as a
// plain file rather than a directory — a typo, a `touch` where `mkdir` was
// meant, or a bad merge. os.ReadDir returns ENOTDIR for this, which is not
// os.IsNotExist, so without special handling it propagates as a bare OS error
// ("open .../environments: not a directory") naming the path but neither the
// expectation nor a suggested action.
...
// TestResourcesDirectoryIsLoaded. A resource declared under resources/ is as real as one
// in infrena.yml, and the two forms coexist — the directory is how a project is ORGANISED, not a
// replacement for the inline block.
func TestResourcesDirectoryIsLoaded(t *testing.T) {
	dir := writeTree(t, map[string]string{
		"infrena.yml":                     "project: p\nresources:\n  inline:\n    type: fake.network\n    cidr: 10.0.0.0/16\n",
	...
	// BOTH: a loader that replaced infrena.yml's resources rather than adding to them would
	// pass a test that only looked for database.yml.
	if !loadedBase(files, "database.yml") {
		t.Error("resources/** was not loaded")
	}
	if !loadedBase(files, "infrena.yml") {
		t.Error("infrena.yml stopped being read")
	}

	// Loading is NOT the feature. A file can be read and then decoded by
	// nothing, leaving `validate` to report the project valid because it
	// validated an empty resource set. Assert the resource is DECLARED, which is
	// what a plan would act on.
...
// organised by region or team nests.
...
// TestDiscoveredDirectoryIsLoaded. Import adds a resource to state, and a resource in state
// that no configuration declares is scheduled for destruction — so the generated file is part
// of the project from the moment it is written.
...
func TestAMissingConventionalDirectoryIsFine(t *testing.T) {
	dir := writeTree(t, map[string]string{"infrena.yml": "project: p\n"})
...
// TestTwoModulesMayEachDeclareTheSameName is the boundary modules depend on: two
...
// TestAnUnquotedInterpolationWithAColonExplainsTheQuoting.
//
...
//
// Do not reach for `errors.Is(err, syscall.ENOTDIR)` here. It is correct on
// Unix and wrong on Windows in both directions: Windows returns
// ERROR_DIRECTORY when a file is read as a directory and Go does not map that
// to ENOTDIR, so the branch never fires; and Go defines
// `syscall.ENOTDIR = ERROR_PATH_NOT_FOUND` there, which is what a MISSING path
// returns, so the same test would be true of a directory that simply was not
// there.
//
// We publish Windows binaries and cannot run the test there from here, so it
// pins the behaviour through a path with no errno in it at all.
...
// TestAMissingConventionalDirectoryIsNotAnError is the other half: absent and
// present-but-wrong-kind must not collapse into one answer. Every one of these
