...
// pseudo-version, which parses as 0.0.0. Reporting one as this build's version makes
// every project stating an `infrena:` floor refuse a developer's own build.
...
// TestAPseudoVersionFromATaggedRepositoryIsNotARelease is the case an all-zero
// check cannot see.
//
// Go derives a pseudo-version from the nearest tag, so once a repository has
// one, a build from any later commit reports v<next>-0.<stamp>-<rev>. That is
// not all zero and it parses cleanly, so a naive check answers it as a release
// — naming a version that has never been published, in exactly the string a
// bug report quotes.
func TestAPseudoVersionFromATaggedRepositoryIsNotARelease(t *testing.T) {
	for _, v := range []string{
		// After a release tag, the form that parses cleanly and is not all zero.
		...
		// A dirty working tree, which is what a developer actually builds. The `+`
