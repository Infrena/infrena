...
// TestRetryableFailsClosedOnUnrecognizedClassification: provider.Retryability
// is a plain uint8, not a validated closed type, so a provider built against a
// future core — or one with a bug — can return a value outside
// {NotSafeToRetry, ConditionallyRetryable, SafeToRetry}. A catch-all default
// that treated such a value as ConditionallyRetryable would have Read and
// Update silently retrying on a classification nobody defined. The core owns
// the policy, and the conservative answer for an unrecognized classification
// is never retry, for every verb — including Read and Update, which have no
// Create/Delete-style asymmetry to fall back on.
...
// TestRetryableRefusesVerbInvalid: VerbInvalid — the zero value of Verb —
// must never be eligible for retry, under any classification including
// SafeToRetry. An uninitialized Verb reaching retryable is itself a bug, and
// the safe answer is "do not retry", not "retry because the classification
// looked fine". Without VerbInvalid the zero value would number the same as
// VerbRead under plain iota, which is the most permissive verb, not the
// least.
...
// "MaxAttempts below 1 means 1". The `< 1` guard in Attempt puts 0 and a
// negative value on the same branch, so this pins the stated rule rather
// than leaving the negative case as an inference.
...
// TestDefaultJitterStaysWithinZeroToD bounds-checks defaultJitter's output and
// additionally requires more than one distinct value across 50 calls. A
// bounds-only check passes a fixed implementation — always d, or always 0 —
// which would defeat the point of jitter: spreading retries out so operations
// that failed at the same instant do not all wake and hammer the provider in
// lockstep. The distinctness check is not flaky in practice, since the draw is
// uniform over a nanosecond-resolution range.
...
// TestAttemptNotifiesEachRetryBeforeWaiting pins OnRetry, which the engine uses
// to emit EventRetrying. Three properties, and the third is the one a careless
...
// An exhausted retry must not read like a single failure. Without the count
// and the time spent, nobody looking at a throttled read can tell whether the
// engine waited and tried five times or never retried at all, and the loop is
