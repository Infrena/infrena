...
// Minimality against an instance's `defaults:` block. An attribute an instance
// already supplies is not something the reader has to write down, so a
// generated file that restates it is a file whose next diff is noise.
...
// `size` is 200 here, which is not the plugin's schema default of 10, so the
// omission can only come from the instance's own block.
...
// The boundary: omitting here would generate a file that plans a change the
...
// The rule that keeps two accounts apart, and the one a map lookup keyed on the
// attribute name alone would break.
//
// Two instances of one plugin exist precisely because they differ. Omitting a
// value because the other account happens to default it writes a file that is
// wrong for the account it describes.
...
// A discovered resource whose instance is unknown must keep everything:
// guessing which block applies is how a value silently disappears from a file
// that then plans a change.
...
// Export is the opposite of minimal: a full record for auditing, where a value
// that happens to match a default today is still a fact about the resource.
// Minimal is what governs the omission, and export does not set it.
