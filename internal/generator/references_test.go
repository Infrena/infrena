...
// The AWS plugin declares list-valued reference edges, so a list of ids must
// project as well as a single one.
...
// The reference resolves at compile time to the same literal the attribute
// held, so the round trip still plans clean.
//
...
// Why the edges come from generation rather than from a second pass over the
// same attributes. An attribute equal to its default is not written at all, so
// the file declares no reference and state must record no edge — a recomputed
// answer that did not know about minimality would record one, and the round
// trip would propose removing it.
