...
func TestABackendThatDiesMidRunErrorsClearly(t *testing.T) {
	...
	// And every later call, rather than one call failing and the next blocking
...
// ends — and `infrena state unlock` prints that PID for a user to go and check.
...
// margin, and every case wants the same binaries. The extra names are hard
// links to the first, because the program decides what to do from the name it
// was launched under, so one build serves them all.
