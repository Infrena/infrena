...
// These are internal/pluginhost's lock cases for a BACKEND binary, and they are
// deliberately the same shape, because the two are the same rule. A backend
// reads and writes the whole of your state, every secret in it included, so an
// unverified backend binary is if anything worse than an unverified provider.
//
...
// asserted "Open returned an error" would pass just as happily against a host
...
// A backend whose binary is not the one plugins.lock recorded is refused
// BEFORE it is launched, and the refusal names the backend.
...
// A hand-placed backend the lock does not mention keeps working (section
// 31.3): the lock governs what INSTALL put on disk rather than claiming
...
// A backend and a provider of the same name are different artifacts — separate
// repositories under section 31.3's convention — and plugins.lock is one file,
// so they cannot share a key. A provider's entry must not be what a backend is
