...
func TestDispatchReplaceCreatePhaseAllocatesANewObject(t *testing.T) {
	...
	// A replace is two nodes at the same address, distinguished only by Phase:
	// OpNode.ID() cannot tell them apart. Destroy first, then create, both
	// carrying Kind == OpReplace.
	...
	// An implementation keying off node.Kind alone would see OpReplace both
	// times and could dispatch the create phase as an Update reusing the
	// original's ProviderID, silently patching the old object instead of
	// replacing it.
