...
// TestADeclaredCeilingOverridesTheDefault, and the floor under it. The number
// comes from a third-party plugin, so a nonsensical claim has to be survivable:
// a ceiling of zero would admit no work at all and the run would hang holding a
// state lock, which is worse than ignoring the claim.
...
// whose plugins predate protocol 5.
