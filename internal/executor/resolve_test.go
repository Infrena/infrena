...
// "environment" the way every real compile does, and reports every resource
// attribute as unavailable, which is what turns a reference into a deferred
// unknown carrying its expression. Building fixtures through the real parser
// and evaluator, rather than a hand-built *value.Expr, is what proves
...
func deferredValue(t *testing.T, src string) value.Value {
	t.Helper()
	e, ds := expressions.Parse(src, value.Origin{File: "infrena.yml", Line: 1})
	if ds.HasErrors() {
...
// afterAttributes stages every not-yet-set Computed schema attribute as
// value.Unknown(kind, SourceProvider) with a nil Expr, purely so planner.Render
// can show "id: (known after apply)". There is no expression to evaluate, so
// resolveAfter must omit the key entirely rather than report the empty result
// as a broken dependency ordering — which would fail the plain create of any
// resource with an unset computed attribute. The provider assigns the value
// and the caller never supplies it.
...
func TestResolveAfterReportsUnresolvedDependencyAsDiagnostic(t *testing.T) {
	...
	// The dependency is missing from the snapshot entirely — the case a bug in
	// dependency ordering would produce.
	got, ds := resolveAfter(op, map[string]*resource.ResourceState{}, "dev")
	if !ds.HasErrors() {
...
func TestResolveAfterMixesVariableAndResourceReference(t *testing.T) {
	// Unknownness is contagious: the whole concat goes unknown because net.id
	// is unresolvable at compile time, even though "environment" resolves
	// immediately.
	//
	// residual() folds "environment" into an OpLiteral as soon as its own
	// compile-time sub-evaluation succeeds, so the Expr stored on this deferred
	// value already has it baked in and runtimeScope.Variable is never
	// consulted here. What this pins is that a residual mixing an already-folded
	// literal with a still-unresolved resource reference evaluates to the right
	// concatenation once the reference resolves — see
...
// runtimeScope.Variable always reports unavailable rather than resolving
// "environment" (or any variable) specially.
//
...
// does for every other test in this file. It hand-builds the malformed shape
// directly, standing in for a compiler bug that lets an unfolded variable
// reference leak into a stored Expr. resolveAfter must not silently resolve
// it: expressions.Evaluate's own "undefined variable" diagnostic must fire and
// the attribute must stay unknown, so the bug surfaces loudly rather than
// being quietly patched over by the scope.
func TestResolveAfterTreatsResidualVarRefAsCompilerBug(t *testing.T) {
	malformed := &value.Expr{
		Op:     value.OpVarRef,
		Ref:    value.VarRef("environment"),
		Origin: value.Origin{File: "infrena.yml", Line: 1},
	}
	op := &planner.Operation{
		Provider: "test",
		Address:  address.Address{Name: "app"},
