...
// prompt on the terminal. A flag answers the question for this command, the
// environment answers it for every command in CI, and the prompt is the
// fallback for a person who has answered it nowhere.
//
// There is deliberately no --vault-password holding the value: a passphrase in
// an argument is in `ps` output for the life of the command and in shell
...
// opening a vault costs one failed command, while a typo when sealing one
// produces a file nobody can open, usually discovered much later with the
// original plaintext gone.
//
