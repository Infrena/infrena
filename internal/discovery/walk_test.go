...
// Naming what it finds is the whole point of Walk over calling Discover
// directly.
func TestWalkNamesEverythingItFinds(t *testing.T) {
	...
	got, problems := Walk(context.Background(), reg, nil, retry.Policy{}, nil)
	if len(problems) != 0 {
...
// Naming happens after the sort rather than during the walk.
//
...
func TestWalkNamesDeterministically(t *testing.T) {
	...
	var first []string
	for i := range 20 {
		got, _ := Walk(context.Background(), reg, nil, retry.Policy{}, nil)
		var names []string
		for _, r := range got {
	...
	// first. Which one gets it is arbitrary; that it is always the same one is
	// not.
...
// What makes the sort in Walk load-bearing, and the only test that can see it.
//
// providers/test returns its own results already sorted, so against that
// provider alone, moving Walk's sort after the naming loop changes nothing. The
// contract is that Walk sorts, not that its callers happen to — a provider is
// free to return results in whatever order its API paginated them, and AWS
// will.
//
...
// A real Discover is API calls; asking a provider about a type it does not
// offer is a round trip whose answer is known in advance.
func TestWalkAsksAProviderOnlyAboutItsOwnTypes(t *testing.T) {
	...
	if _, problems := Walk(context.Background(), reg, []string{"spy.thing"}, retry.Policy{}, nil); len(problems) != 0 {
		t.Fatalf("unexpected problems: %v", problems)
	}
	if len(spy.asked) != 1 {
	...
	spy.asked = nil
	if _, problems := Walk(context.Background(), reg, []string{"other.thing"}, retry.Policy{}, nil); len(problems) != 0 {
		t.Fatalf("unexpected problems: %v", problems)
	}
	if len(spy.asked) != 0 {
...
// "Found nothing" and "could not look" are different answers, and a user
...
func TestWalkRetriesADiscoverTheProviderCallsSafeToRetry(t *testing.T) {
	...
	got, problems := Walk(context.Background(), reg, nil, policy, nil)
...
// A sweep must not be silent. One Discover call covers every type an instance
// serves — hundreds of them for AWS — and the plugin does the whole thing
// internally, so the host cannot say which type it is on. What it can say is
// that the sweep started, how much it asked for, and that it is still running;
// the renderer's heartbeat does the rest. Without this, discover and import
// print nothing at all for minutes, which is indistinguishable from a hang.
func TestWalkReportsEachInstancesSweep(t *testing.T) {
	prov := &recordingProvider{returns: []provider.DiscoveredResource{
		{Type: "spy.thing", ProviderID: "id-1"},
		{Type: "spy.thing", ProviderID: "id-2"},
	}}
	reg := registry.New()
	if err := reg.Register("spy", prov); err != nil {
		t.Fatalf("Register: %v", err)
	...
	var got []Progress
	Walk(context.Background(), reg, nil, retry.Policy{}, func(p Progress) { got = append(got, p) })

	if len(got) != 2 {
		t.Fatalf("got %d progress notifications, want a start and a finish: %+v", len(got), got)
	}
	start, done := got[0], got[1]
	if start.Instance != "spy" || start.Done {
		t.Errorf("first notification is not the start of spy's sweep: %+v", start)
	}
	if start.Types == 0 {
		t.Error("the start does not say how many types were asked for, which is the only size a reader gets")
	}
	if !done.Done || done.Instance != "spy" {
		t.Errorf("second notification is not the finish: %+v", done)
	}
	if done.Found != 2 {
		t.Errorf("Found = %d, want 2", done.Found)
	}
}

// A sweep that fails still finishes, so the renderer can drop it from the
// heartbeat rather than leaving it "still working" forever.
func TestWalkReportsAFailedSweepAsFinished(t *testing.T) {
	prov := &recordingProvider{fail: errors.New("credentials expired")}
	reg := registry.New()
	if err := reg.Register("spy", prov); err != nil {
		t.Fatalf("Register: %v", err)
	}
	...
	if len(got) != 2 || !got[1].Done {
		t.Fatalf("a failed sweep did not report a finish: %+v", got)
	}
	if got[1].Err == nil {
		t.Error("the finish does not carry why the sweep failed")
	}
}
