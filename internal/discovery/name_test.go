...
// The fallback is what a person pastes into a console to find the thing again,
// so it stays spelled the way the provider spells it wherever that is legal.
...
// keeping its own copy of the rule. This is what notices if it ever becomes a
// second copy.
...
// Two resources tagged `orders` are two resources. A silently merged pair is a
// resource dropped, and a resource dropped out of a survey is one the next plan
// proposes destroying.
...
// The case the loop in Unique exists for: a resource already named
// `orders_db-9` and one tagged `orders` with ID `db-9` generate the same
// candidate.
...
// A tag of "", "  ", an unknown value or a non-string is not a name, and must
// not produce an empty or invalid one.
...
// The same resource named twice from a fresh table is the same name. Attribute
// lookup walks a fixed list rather than a map, because Go's map iteration is
// randomised.
...
// The tag map is found through the schema's alias fold, never by a literal
// `tags` key. The AWS plugin spells the attribute `Tags`, so a literal lookup
// misses on every AWS resource and every name falls through to the sanitised
// provider ID.
...
func TestTypePrefixIsTheLastDottedSegment(t *testing.T) {
	// The AWS generator already collapses unambiguous type names, so the last
	// segment is the right prefix without the plugin having to publish a short
	// name and without a protocol change.
...
// suffix only fires for two resources of one type sharing a Name tag, which is
// genuinely two resources and must never be silently merged.
