...
// The host verifies against the lock on every launch once a lock exists, so a
// binary replaced on disk after it was installed is caught at the next command
// rather than never.
//
...
// is the only property worth checking here. A test that only asserted "Load
// returned an error" would pass just as happily against a loader that ran the
...
// A hand-placed binary the lock does not mention keeps working: the lock governs
// what install put on disk rather than claiming authority over everything.
