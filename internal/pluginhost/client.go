...
//
// Several guarantees used to rest on each provider following a doc comment. A
// binary somebody else built cannot be held to a comment, so each one lives
// here, where it holds for every plugin including the official ones.
package pluginhost
...
// MaxConcurrency is the ceiling this plugin declared, or 0 for no claim. See
// pluginproto.Handshake.MaxConcurrency.
...
// Create can orphan a resource that was really created, so the host always waits
// for the plugin's own answer about what happened.
func (c *Client) call(ctx context.Context, method string, params any, out any) error {
	...
	case <-ctx.Done():
		...
		// Keep waiting, so the host always learns what the plugin actually did.
		resp := <-ch
		// A successful answer means this call succeeded: the resource really was
		// created, and returning ctx.Err() instead would tell the executor the
		// operation failed, which is how a created resource ends up recorded
		// nowhere. The caller's own context is already cancelled.
		//
		// The executor dispatches every provider call on a context.WithoutCancel,
		// so nothing cancels mid-flight today; this path keeps that true for a
		// caller that does.
		return decodeResponse(resp, method, out)
	}
}
...
//
// The retry logic reads Retryability from here rather than calling
// ClassifyError, which cannot cross a process boundary.
type ProviderError struct {
	// Message is what the plugin said went wrong.
	Message string
	// Retryability is the plugin's own classification of the failure.
	Retryability provider.Retryability
}

// Error returns the message the plugin reported.
