...
func (p *Plugin) loadSchemas(ctx context.Context) error {
	...
	// ValidateAll rather than a Validate() per definition, and AFTER the loop above,
	// which is what makes the nil check that loop performs load-bearing: ValidateAll
	// reads d.Type on every entry, so a nil would panic here rather than producing
	// the loop's "sent an empty resource definition". An earlier draft of this had
	// the call above the loop with a comment claiming it was below — the comment was
	// the intent and the code was the bug, and it panicked on a null array element.
	//
	...
	//
	// Running last also fixes the order a plugin author meets its problems in: a type
	// outside the plugin's prefix, or an attribute named after a lifecycle option, is
	// a more basic complaint than a malformed attribute somewhere else in the set, and
	// is now reported first.
	//
