...
type Plugin struct {
	...
	// plugin needs it — a relative `cloud:` path resolves against the project,
	// not against whatever working directory the plugin inherited — but it is
	// not part of anybody's configuration, and a reserved key travelling through
	// those maps would be one every diagnostic and every `defaults:` check had
	// to learn to ignore.
	dir string
}
...
//
// On load, through the same checks RegisterPlugin applies to a built-in, plus
// one that only matters for a binary somebody else built: a plugin's types must
// be prefixed with its own name.
func (p *Plugin) loadSchemas(ctx context.Context) error {
	...
	for _, d := range result.Definitions {
		...
		if reserved := reservedAttributeOf(d); reserved != "" {
			// prevent_destroy and retain belong to infrena's lifecycle handling
			// and are accepted in a provider instance's `defaults:` for every
			// resource, so an attribute of either name would make one key mean
			// two things. Refused here as well as in the registry, so the
			// message names the plugin and arrives when its schemas load.
			return fmt.Errorf(
	...
	// ValidateAll rather than a Validate() per definition: it covers the
	// relationships between definitions, which no single one can check — an
	// attribute's References names another type, and whether that type exists is
	// a fact about the whole set.
	//
	// It must run after the loop above, which is what makes that loop's nil
	// check load-bearing: ValidateAll reads d.Type on every entry, so a nil
	// definition would panic here instead of producing the loop's message.
	// Running last also puts the more basic complaints first.
	//
	// It belongs in the adapter rather than in pkg/plugintest, which reaches
	// this same code through InProcess: validating there would be a second
	// implementation, and the drift between them would be invisible.
...
//
// Every method here is the same shape: send what the plugin owns, and rebuild
// what comes back under the host's own rules. There is deliberately no path from
// a plugin's answer to the engine that skips it.
...
//
// provider.ClassifyError takes an `error`, which does not cross a pipe, so the
// plugin classifies on its own side and the answer travels on the error. An
// error from anywhere else is not safe to retry: a host-side failure — a broken
...
// plugin does not own back on the result itself, so carrying bookkeeping forward
// is not a plugin obligation: a field never sent cannot be dropped, and losing
// Lifecycle would make a prevent_destroy guard vanish silently.
...
func (r *remoteProvider) Create(ctx context.Context, desired *resource.DesiredResource) (*resource.ResourceState, error) {
	...
	// A create has no prior state to re-attach bookkeeping from, so it comes from
	// the desired resource. The address is the load-bearing part: state.Set drops
	// a resource state with no address, so the run would report "0 applied" for
	// an operation whose provider really did create something.
	//
	// Lifecycle is deliberately not taken from desired: the executor stamps it
	// onto the returned state from the plan, which is the one place that knows
...
// destroy, so saying so is the only useful thing left to do.
...
func (r *remoteProvider) rebuild(resourceType string, result pluginproto.ResourceResult, carry *resource.ResourceState) (*resource.ResourceState, error) {
	...
	out := &resource.ResourceState{
		...
		Attributes: attrs,
		// Set unconditionally, because this object is one instance and knows
		// which, while a plugin cannot know which instance of itself it is. The
		// executor stamps it for create and update, but `import` writes straight
		// to state, so leaving it to a later caller means an adopted resource
		// arrives with no instance and the next plan finds no provider for it.
		Provider: r.instance,
	}
	if carry != nil {
...
func (r *remoteProvider) check(def *schema.ResourceDefinition, attrs map[string]value.Value) (map[string]value.Value, error) {
	...
	for _, name := range names {
		...
		// plugin: one that forgets the flag must not be able to put a password
		// into a plan, a report or a generated file.
		if attr.Sensitive {
		...
		// wrote it, which would make a plan credit the wrong file.
