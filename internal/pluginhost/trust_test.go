...
//
// Every rule here once rested on a provider following a doc comment. A binary
...
// The most important rule here. `infrena refresh` persists whatever Read returns, so
// a provider that silently dropped Lifecycle would make a configured prevent_destroy
// guard vanish with no error at all.
//
// The bookkeeping fields are never sent, so they cannot be dropped: the host
// re-attaches them from what it already holds.
...
// Provenance is the host's: a plugin reports a fact about the remote system, so the
// value came from the provider. A plugin claiming SourceExplicit would make a plan
// credit the wrong file, and `[--var]` beside a value nobody passed is worse than no
// provenance.
...
// Refused rather than persisted into state, where it would show up in a plan as a
// change nothing can explain.
...
// Without the prefix rule a plugin could serve `aws.instance` and quietly take over
// another plugin's resources; the registry's duplicate-type check would then refuse
// whichever loaded second, making the outcome depend on ordering.
...
// A renamed or mis-copied binary.
...
//
// internal/executor dispatches provider calls on a context.WithoutCancel for the
// same reason: an in-flight create must not be told to abort mid-call.
...
// ClassifyError takes an `error`, which does not cross a pipe, so the retry logic
// reads what the plugin sent instead.
...
// The boundary of the rule above. A broken pipe or a dead plugin is exactly the case
// where retrying a Create could create a second resource.
...
// instance's `defaults:` accepts them for every resource — so an attribute of either
// name would make one key mean two things.
//
...
// The SDK's stray-print guard, `os.Stdout = os.Stderr` in pluginsdk.Main, is not
// reachable from here: InProcess calls Serve directly, and Serve writes to the pipe
// it is given, so a plugin's fmt.Println goes to the test's own stdout and nowhere
// near the stream. The guard cannot move into Serve either, which is the obvious
// "fix" — under InProcess, Serve runs in the host's process, and redirecting the
// process-wide os.Stdout there would clobber the host's own output, which is where
// a plan gets printed. It is only observable in a real subprocess, where stdout is
// the pipe.
//
...
func TestGarbageOnTheStreamFailsLoudlyRatherThanHanging(t *testing.T) {
	...
	// error it is trying to provoke later. A fake that waits to be asked cannot
	// reorder the two.
