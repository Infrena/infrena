...
func TestDecodeRejectsFutureVersion(t *testing.T) {
	...
	if _, err := Decode(data); err == nil {
		t.Error("state written by a newer infrena must be refused, not silently misread")
	}
}

// The migrations these tests register go from 0 to CurrentVersion in one step, rather
// than to a literal 1. They are about the chain's behaviour, not about any particular
// version, so they must not need editing when CurrentVersion moves.
...
// TestStateRefusesToWriteAPendingExpression is the only thing asserting this rule.
// pkg/value serialises expressions, because a plan artifact has to carry them, so state
