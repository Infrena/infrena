...
func TestSecondLockOnSameEnvironmentFails(t *testing.T) {
	...
	if err == nil {
		t.Fatal("a second lock on the same environment must fail — only one holder at a time")
	}
...
func TestLockErrorNamesTheHolder(t *testing.T) {
	...
	for _, want := range []string{"held by", "pid"} {
		if !strings.Contains(strings.ToLower(msg), want) {
			t.Errorf("conflict message %q does not say %q — it must identify who holds the lock", msg, want)
		}
	}
}
...
func TestConcurrentLockAttemptsElectExactlyOneWinner(t *testing.T) {
	// The other lock tests are single-goroutine: a stat-then-create
	// implementation would pass every one of them. This is the only test
	// that exercises the race the atomic create exists to prevent.
	//
	// One election is a coin flip against a real TOCTOU, which shows up in
	// only some interleavings, so the property is measured across many
	// independent rounds rather than assumed from one. Do not reduce this
	// loop back to a single round.
	b := NewLocal(t.TempDir())
	ctx := context.Background()
	...
	for round := 1; round <= rounds; round++ {
		...
		if granted != 1 {
			t.Fatalf("round %d: %d goroutines acquired the lock, want exactly 1 — only one holder at a time", round, granted)
		}
...
// that an exclusive create alone does not provide: it makes acquisition
// atomic, but if the file's content is written after creation there is a
// window where the file exists and is empty. An Inspect landing in that
// window reads zero bytes and reports a malformed lock file — reachable,
// since Lock itself calls Inspect on the already-exists path to name the
// holder.
//
// Inspect has exactly two correct outcomes: no lock, or a complete one. A
// malformed-JSON error is a third that must never happen, however the two
// calls interleave. The window is a handful of instructions wide, so many
// goroutines run against many rounds to land in it.
...
// mode a naive fix for the above would introduce: writing a temp file and
// renaming it over the lock, mirroring Put's pattern in local.go. Rename
// overwrites its target, so a second Lock would silently replace the first
// holder's file instead of being refused, and two concurrent applies would
// both believe they hold the lock. os.Link keeps exclusivity — it fails
// when the target exists — while still writing content atomically.
//
// The contract itself is pinned elsewhere too; it is restated here against
// the Link-based implementation so a future rewrite of Lock's atomicity
...
func TestSecondLockStillRefusedAndNamesHolderAfterLinkFix(t *testing.T) {
	...
	_, err := b.Lock(ctx, "production")
	if err == nil {
		t.Fatal("a second Lock on a held environment must fail — only one holder at a time")
	}
	if !errors.Is(err, ErrLocked) {
		t.Errorf("error = %v, want one wrapping ErrLocked", err)
	}
	...
	for _, want := range []string{"held by", "pid"} {
		if !strings.Contains(msg, want) {
			t.Errorf("conflict message %q does not say %q — it must identify who holds the lock", err.Error(), want)
		}
	}
