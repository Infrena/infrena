...
// The fake provider's rename from `test` to `fake`, and the first migration to run.
//
// Keep search-and-replace away from this file. Both the fixture and the inline document
// below deliberately hold `test.*`: they are the INPUT to the rename, so rewriting them
// to `fake.*` makes every assertion here vacuously true. The guards in migrationFixture
// exist to catch that.
//
// The fixture is testdata/state-v1.json, the frozen golden a version-1 infrena actually
// wrote, kept rather than regenerated when the golden moved on. It carries a 2^53+1
// integer, so it also proves the non-lossy migration decode on a real file.
...
func TestTheMigrationPreservesEverythingItIsNotRewriting(t *testing.T) {
	...
	// The 2^53+1 integer is why this fixture is the right one: it is the only real file
	// the suite reads through the migration path, where rounding would be silent.
