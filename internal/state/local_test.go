...
func TestPutIsAtomicAndPrivate(t *testing.T) {
	...
	if perm := info.Mode().Perm(); perm != 0o600 {
		t.Errorf("state file mode = %o, want 600 — state holds sensitive values", perm)
	}

	// The state directory should hold exactly the state file Put wrote and
	// the lock file held across it. os.ReadDir sorts by filename, so pinning
	// the names as well as the count matters: a stray "dev.json.tmp" left
	// behind alongside a missing "dev.lock" passes a bare length check.
...
func TestEnvironmentsAreIndependent(t *testing.T) {
	...
	if len(prod.Resources) != 0 {
		t.Error("environments must have completely independent state")
	}
}
...
func TestPutWithoutLockRefuses(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Put without a held lock must refuse — the one-holder-at-a-time rule must hold at the point state is written, not only where a caller happened to request a lock upstream")
	}
	...
	// Put was called without ever calling Lock, so there is no lock to
	// unlock and suggesting `infrena state unlock` would mislead. That
	// remedy belongs only on the refusal below, where a lock genuinely
	// exists and might be stale.
	if strings.Contains(err.Error(), "infrena state unlock") {
		t.Errorf("error = %q, must not suggest `infrena state unlock` — no lock exists to unlock when Put is called without ever calling Lock", err.Error())
	}
}
...
func TestPutRefusesWhenLockedByAnotherProcess(t *testing.T) {
	...
	// refusal must name the concrete remedy the way Unlock's own conflict
	// message does.
	if !strings.Contains(err.Error(), "infrena state unlock production") {
		t.Errorf("error = %q, want it to suggest `infrena state unlock production` as the remedy for a stale lock held by another process", err.Error())
	}
}
...
func TestMultiplePutsSucceedUnderOneHeldLock(t *testing.T) {
	...
	// whole run. That is the shape that matters, not a single Put
	// immediately after a single Lock.
	b := NewLocal(t.TempDir())
	ctx := context.Background()
	if _, err := b.Lock(ctx, "dev"); err != nil {
		t.Fatalf("Lock: %v", err)
