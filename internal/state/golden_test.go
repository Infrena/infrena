...
//
// Old goldens are kept rather than regenerated: testdata/state-v1.json is the real
// bytes a version-1 infrena wrote, and the migration tests read it. A hand-written
// approximation of a retired format proves much less than the format itself.
...
// a checked-in file. Determinism tests compare encodings to each other, so they
// cannot see the on-disk shape change. The format is a versioned contract, and
// renaming a Go field must break this test rather than silently rewrite
// everyone's state.
