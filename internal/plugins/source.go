// Package plugins knows where a plugin may come from, and which of those places
// the user has trusted.
//
// It makes no network requests; the client that does lives in the subpackage
// remote. The network is never on the hot path, and keeping every request in one
// place above this package is what makes that rule checkable.
//
...
//
// Not called Kind, because Kind is taken in this package and means whether a
// source names an owner or one repository. The user-facing flag is still
// `--kind`, since that is the word a person types.
type Role int
...
type Source struct {
	...
	// value there, because an owner publishes both roles.
	Role Role
}

// ParseSource reads the plugin source syntax.
//
