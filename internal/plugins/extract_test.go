...
// buildTar builds a gzipped tarball, which is the shape a release ships for
...
// The release workflow packs the binary inside a versioned directory, so the
// entry an archive really carries is nested rather than top level. An extractor
// that matched the full path would work against no actual release.
...
// An entry whose path leaves the destination fails the whole archive - not
// sanitised, not skipped. An extractor that quietly drops this one will happily
...
// buildTarWithDirectory builds the archive a release actually ships: tar writes
// an entry for the versioned directory it was handed, not only for the files
// inside it.
...
// The archive a real release ships. A tarball built from a directory carries an
// entry for the directory itself, so an extractor that refused everything that
// was not a regular file would refuse every genuine install.
