...
// Every filtered-out candidate is worth mentioning, with the reason. A user
// whose plugin exists but has no darwin/arm64 build must be told that, not told
// nothing was found — those send a reader to completely different places.
