...
//
// IT IS AN OPTIMISATION, AND IT BEHAVES LIKE ONE. Every failure on the read
// path is a MISS rather than an error, because a cache that can fail a command
// is worse than no cache at all: the command would then break for a reason that
// has nothing to do with what the user asked.
type Cache struct {
...
// entry is what is stored: the payload and when it was stored. The time lives
// WITH the payload rather than in the file's mtime, because an mtime is
// rewritten by ordinary tools (a backup restore, a copy) and would silently
// make a stale answer look fresh.
type entry struct {
...
//
// A failure is REPORTED so a caller can say so if it wants to, but no caller
// should treat it as fatal: failing to write a cache entry has no effect on the
...
//
// THE KEY IS HASHED, NOT SANITISED. A key carries a slash and whatever an owner
// called themselves, and hashing removes the traversal question entirely rather
// than answering it: there is no sequence of characters that turns into a
// parent directory, and two keys that differ at all land in two files.
