...
// fixture that says yes to anything: a server that returns the same body
// whatever was asked for passes against a client calling entirely the wrong
// endpoint.
//
// The paths are the ones api.github.com actually serves. A release is read at
// /repos/{owner}/{repo}/releases/tags/{tag}, and an asset's bytes come from
// /repos/{owner}/{repo}/releases/assets/{id} with an octet-stream Accept. A
// request for anything else gets the 404 the real forge would give.
...
// The asset name convention mirrors infrena's own releases.
...
// A backend is not named like a provider: its asset is named after the backend
// binary. Pasting infrena-plugin- in front of every name asks for an asset no
...
func TestChecksumsParsesSHA256SUMS(t *testing.T) {
	// The ./ prefix is what `sha256sum ./*` writes, so a parser that keys on
...
// The same mistake on the download path: a condition meaning "could not see"
// reported as "it is not there". A rate limit while fetching a release must
// stay a rate limit, or a user is sent to check a spelling that was right.
