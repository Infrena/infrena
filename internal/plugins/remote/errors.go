...
// Both variables are named in error messages, because setting one is what fixes
// a rate limit today rather than in an hour.
const (
	// TokenVar is the environment variable infrena reads first for a forge
	// token.
	TokenVar = "INFRENA_GITHUB_TOKEN"
	// GitHubTokenVar is the variable infrena falls back to when TokenVar is
	// unset.
	GitHubTokenVar = "GITHUB_TOKEN"
)

// RateLimitError says the forge refused because this caller has asked too often,
// not because the thing asked for is missing.
//
// Keeping the two apart matters: an unauthenticated caller gets sixty requests
// an hour, which a single owner search can spend, and reporting that as "not
// found" tells a user their plugin does not exist, sends them to check a
// spelling that was right, and hides a condition that clears itself within the
// hour.
...
// collapsing it into either of the two above sends the reader somewhere useless.
