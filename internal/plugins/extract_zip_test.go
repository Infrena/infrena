...
// The windows half of every test in extract_test.go. A release zips the windows
// builds and tars everything else, so install is only covered on every platform
// if both formats are.
//
// Zip is not a thinner threat model than tar: it has its own long history of
// traversal bugs, it stores a Unix mode so it can encode a symlink, it permits
// two entries with the same name, and its headers declare an uncompressed size
// that a bomb simply lies about. So this is the same table, entry for entry,
// built against archive/zip.
...
// versioned directory is an entry of its own, because zip writes one for the
...
//
// Two different guards are in play. A bomb that declares its real, enormous
// size is refused by this package's own cap on written bytes; the lie here is
// caught one layer lower, because archive/zip stops a reader at the size the
// entry declared, so the extra bytes never arrive and the entry fails as a
// malformed zip. Both are refusals with nothing written, which is what this
// pins.
