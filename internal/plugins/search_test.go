...
//
// THE FAKE IS THE POINT. Search takes a Fetcher so that it depends on the SHAPE
// of the forge calls and never on making one, which is what lets every test
// here run with no network at all and what keeps this package's dependency
// graph free of net/http. A test that needed a server would be a test of the
// wrong thing.
...
// read from runtime, so these tests answer the same on every developer's
// laptop and in CI.
...
// A candidate that cannot run here is still returned, with its reason. This is
// the whole point of Task 3 reaching the search: "no build for your machine" and
// "no such plugin" are different answers, and dropping the first would render
// it as the second.
...
// one search render identically. ORDERING IS PRESENTATION ONLY: being first
