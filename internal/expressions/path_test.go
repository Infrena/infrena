...
func TestAPathOutOfAContainerMarkedSensitiveStaysSensitive(t *testing.T) {
	// CarrySensitivity sets Sensitive on the CONTAINER as well as its leaves, so
	// a sensitive map variable holds the flag at the top and may hold nothing on
	// the leaf. Returning the leaf as found declassifies it, and value.Format
	// then prints it in clear.
