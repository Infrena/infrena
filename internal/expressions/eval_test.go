...
func evalSrc(t *testing.T, src string, scope Scope) (value.Value, diag.Diagnostics) {
	t.Helper()
	e, ds := Parse(src, value.Origin{File: "infrena.yml", Line: 1})
	if ds.HasErrors() {
...
// TestConcatOfAListIsAnErrorNotAnEmptyString guards against a fabricated empty
// string: evaluateConcat's stringify helper, given a composite, can fall through
// to AsString on a non-string Raw and return "", which then splices silently
// into the result. Unknownness must never substitute a zero value. Schema
// binding is meant to reject composites inside interpolations before evaluation
// runs; this is the backstop if one ever reaches here.
...
// TestDefaultWithUnknownFallbackCarriesTheWholeCallExpr. evaluateCall must
// override the Expr of an unknown result. defaultFunc, when its primary argument
// is unknown (or blank) and its fallback is itself unknown, returns the
// fallback's own Value — whose Expr points only to that sub-expression, not to
// the default(...) call. Re-evaluating that narrower Expr once the fallback
// resolves would skip the primary-vs-fallback logic entirely, discarding
// default's semantics the moment the primary attribute also becomes known.
...
//
// Both assertions are UNCONDITIONAL on purpose. Guarding the second with
// `if args[0].Op == OpLiteral` makes it vacuous: it could then never fail.
...
// case where nothing resolved. It guards against the fold damaging an expression
// it should leave alone; it is not evidence the fold works.
...
// TestFoldedSensitiveLiteralRedactsInStringRendering. Folding a resolved
// SENSITIVE value into an OpLiteral inside a deferred expression makes it
// reachable by Expr.String() and inner(), which therefore have to check
// Literal.Sensitive themselves. String() is not the sanctioned redaction path —
// value.Format is — so a second, unguarded rendering path is how a secret leaks.
...
// evaluateConcat does. Deferring the whole SOURCE expression instead leaves a
// resolved argument as an OpVarRef — ref NAME only, no value — and ConfigHash
// cannot see a change through one of those.
...
// call shape that must NOT fold: default()'s own unknown-out deferral. It holds
// also when the PRIMARY resolved to a known, blank value — default() still
// routes to the fallback, the fallback is still unknown, and folding a literal
// "" in would tell ConfigHash nothing a non-blank value could not already show
// by taking the fully-resolved path. residual leaves this call's source
...
// the evaluator's sensitivity union recursive would be a regression rather than
// a fix.
//
