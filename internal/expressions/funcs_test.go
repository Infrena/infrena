...
func TestSensitivityUnionsEveryArgumentPosition(t *testing.T) {
	// Every argument position counts. A hand-written union that omits one — a
	// separator, a search string — lets a secret search term reveal its own
	// position through an unclassified result, a side channel on its content.
	secret := str("hunter2").WithSensitive(true)
	plain := str("plain")
...
func TestMalformedCompositeIsTreatedAsSensitive(t *testing.T) {
	...
	//
	// Asserted through anySensitive rather than the predicate directly, which
	// lives in pkg/value and is tested there. What this pins is that the
	// classification the built-ins get is still the conservative one.
...
// deliberate configuration-language change rather than a side effect. Update it
// only when the language itself gains a function.
...
// TestFunctionsDoNotClassifyValuesThatAreNotSensitive is the deliberate inverse
// of the union test above, which — like every other redaction test here —
// asserts only that secrets are NOT shown. Without this, sensitivity is pinned
// in one direction only, and a predicate hard-wired to return true leaves the
// whole suite green.
//
// Over-classification is not a security bug, which is why the instinct guarding
// this area points away from it. Its cost is different in kind: every
// transformed value in every plan prints <sensitive>, and plans stop being
// readable.
//
// The two tests together are what discriminate: this one fails if
// classification becomes too broad, its sibling fails if it becomes too narrow.
// Either alone can be satisfied by a constant.
...
// TestMergeKeepsSensitivityPerLeaf asserts both halves. A wholly-redacted result
// hides the keys a reader needs to act on; an unclassified one puts a password
// in a tag.
