...
func mustParse(t *testing.T, src string) *value.Expr {
	t.Helper()
	e, ds := Parse(src, value.Origin{File: "infrena.yml", Line: 3})
	if ds.HasErrors() {
...
func TestParseReportsUnclosedInterpolation(t *testing.T) {
	_, ds := Parse("${database.endpoint", value.Origin{File: "infrena.yml", Line: 3})
	if !ds.HasErrors() {
...
func TestParseDiagnosticsNameTheOffendingSource(t *testing.T) {
	_, ds := Parse("${lower(}", value.Origin{File: "infrena.yml", Line: 7})
	if len(ds) == 0 {
		t.Fatal("expected a diagnostic")
	}
	var rendered strings.Builder
	ds.Render(&rendered)
	if !strings.Contains(rendered.String(), "infrena.yml:7") {
...
// TestReferenceIntoAModuleIsRefused. ${module.prod.database.id} parses to the
// target name "module.prod.database", and
// address.Address{Name: "module.prod.database"}.String() returns that verbatim —
// byte for byte what Address{Module: ["prod"], Name: "database"} renders for the
// REAL database inside instance prod. Stage 6 keys its target map by canonical
// address, so without this guard the two collide and a user can reach inside a
// module. A module exposes outputs, not resources.
//
...
// qualifier. Under this grammar the target is always segments[0], so `module` is
// only ever a qualifier there; elsewhere it is an ordinary path step.
...
// being satisfied by rejecting everything. ${prod.endpoint} is how a caller
// reads a module instance's output; a guard that refused it would make modules
...
// TestModuleIsOrdinaryOutsideTheTargetSegment. A guard that scanned every
// segment instead of just segments[0] would refuse `${prod.module.net.vpc.id}`
// as reaching into a module, even though "module" there is only ATTRIBUTE and
// PATH text — a map key or a path step the user wrote, never an address
// qualifier. Under this grammar the target is always exactly one segment, so
// `module` can only legitimately qualify a reference at position 0.
func TestModuleIsOrdinaryOutsideTheTargetSegment(t *testing.T) {
	for _, src := range []string{
		...
		t.Run(src, func(t *testing.T) {
			_, ds := Parse(src, value.Origin{File: "infrena.yml", Line: 3})
			if ds.HasErrors() {
				t.Fatalf("Parse(%q) errored: %v", src, ds)
			}
		})
...
func TestResourceNamedModuleIsRefusedWithItsOwnReason(t *testing.T) {
	_, ds := Parse("${module.id}", value.Origin{File: "infrena.yml", Line: 3})
	if !ds.HasErrors() {
...
func TestQualifiedReferencesDoNotRouteThroughTheParser(t *testing.T) {
	...
	if _, ds := Parse("${"+qualified.String()+"}", value.Origin{File: "infrena.yml", Line: 1}); !ds.HasErrors() {
...
func TestEmptyKeyBeforeABracketIsMalformed(t *testing.T) {
	_, ds := Parse("${var.a.[0]}", value.Origin{File: "infrena.yml", Line: 3})
	if !ds.HasErrors() {
...
func TestBracketOnTheTargetIsMalformed(t *testing.T) {
	_, ds := Parse("${vpc[0].id}", value.Origin{File: "infrena.yml", Line: 3})
	if !ds.HasErrors() {
