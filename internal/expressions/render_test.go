...
// TestTemplateFuncNamesIsPinned. The set is ours rather than sprig's, and
// deliberately tiny: sprig ships hundreds of functions, several of which
// contradict guarantees this project asserts — env and expandenv read the
// environment, so a secret would reach rendered text with its sensitivity
// stripped; uuidv4, now and the rand family break plan determinism;
// getHostByName does a DNS lookup while rendering. A denylist against an API
// that grows on someone else's schedule fails silently, so the set is built up
// instead of cut down.
//
