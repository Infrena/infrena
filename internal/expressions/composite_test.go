...
// What happens to a leaf is the caller's business; these tests cover only that
// every leaf is reached, structure survives, and keys are left alone.

// marker replaces a leaf with a marker naming what it saw, so a test can assert
...
// TestTheWalkPreservesLeafSensitivity. pkg/value models sensitivity per leaf and
// value.Format redacts at that granularity. The walk must not flatten either: a
// map with one secret leaf stays a map with one secret leaf, so a plan shows the
// other keys.
//
// Both halves are asserted, because neither alone is enough — a wholly-redacted
// map hides what a reader needs, and a wholly-visible one leaks.
