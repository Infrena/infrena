...
func origin() value.Origin { return value.Origin{File: "infrena.yml", Line: 3} }
...
// TestResolveDeferredLeavesAComputedAttributeAlone. An unknown with no
// expression is not a deferred reference: it is a computed schema attribute the
// provider assigns itself, staged as unknown so a plan can render "(known after
// apply)". Evaluating it calls Evaluate(nil, scope), which returns an unknown
// with NO error, so it would silently be reported as unresolved — and the
// executor turns "unresolved" into a hard failure. Every create of any resource
// with an unset computed attribute would fail.
...
// trusting a lucky map iteration. It loops over deliberately reversed input,
// because Go's randomised map order lets an unsorted implementation pass often
// enough that a single run proves nothing.
...
// masks a violation of this — the referring value keeps its own expression
// either way — so the guarantee has to be tested where it is made rather than
// through its caller. An unknown is not an answer: any other caller reaching for
// this scope would otherwise be handed one resource's "I do not know yet" as
// though it were another's value.
...
// TestAttributeResolvesWithinItsModuleAndNotAgainstASameNamedRootResource.
//
...
//
// A lookup keyed on the bare name resolves the module reference to the ROOT db
// and hands back "root.example.com" without erroring: the plan is written with
// another resource's endpoint in it, and apply carries it out. That is why the
// fixture gives the two DIFFERENT values; two identical ones would pass against
// the broken lookup.
...
// never qualifies.
//
// Carrying a module path on Reference is only half of it. If bindAttribute
// parses and evaluates without calling Qualify, every reference stays
// scope-relative with an empty module path and reaches this lookup looking
...
// try the canonical key, then match on Name alone. That makes the symptom go
// away and silently restores the cross-module mismatch — two modules each
...
func TestAttributeRefusesAnUnqualifiedReference(t *testing.T) {
	...
	if v, ok := scope.Attribute(unqualified); ok {
		...
		t.Errorf("an unqualified %s resolved to %q against a snapshot holding only module.net.db. "+
			...
			"grown a bare-name fallback — which restores the cross-module mismatch this test exists "+
			"to prevent.", unqualified, s)
	}
...
func TestAPathIntoAnUnresolvedResourceStaysDeferredWithItsExpression(t *testing.T) {
	// The property `apply --plan` depends on: an unknown must carry the
	// expression that reproduces it, or a saved plan applies with the attribute
	// silently unset.
