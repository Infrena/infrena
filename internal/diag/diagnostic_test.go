...
func TestCollectsRatherThanStopping(t *testing.T) {
	...
	if len(ds) != 2 {
		t.Fatalf("len = %d, want 2 — validate reports every problem in one pass", len(ds))
	}
}
...
func TestRenderNamesTheModuleChain(t *testing.T) {
	...
	if !strings.Contains(out.String(), "module.platform.module.database") {
		t.Errorf("module chain missing — flattening makes this the only way to locate the instantiation\n%s", out.String())
	}
}

// TestSeverityStringNamesUnknownValues pins the explicit default. Answering
// "Error" for every value that is not SeverityWarning makes a corrupt severity
// indistinguishable from a real error, and it reaches the persisted plan
// artifact through the planner's wire form.
