...
// rejects it.
//
// That check is the point of the helper, not a convenience. A fixture rejected
// by an earlier stage never reaches the stage-5 guard its test names, so
// deleting that guard would change nothing. A fixture that does not decode
// cannot pin anything in this package.
...
// need one, because internal/modules/source owns that behaviour and tests it
...
func (paths) Resolve(s source.Source, baseDir string) (source.Resolution, diag.Diagnostics) {
	...
	// No Commit: a local path has no revision to pin, which is why a pin is
	// required on remote sources and not on these.
