...
func TestUnsuppliedInputTakesTheDeclaredDefaultAtScopeModuleDefault(t *testing.T) {
	...
	// The rung reserved for a module's own defaults.
	if got.Scope != value.ScopeModuleDefault {
...
// Stage 5 keeps whatever provenance evaluation produced; it does not re-stamp.
...
// The ordering fixture. `size` is bound in BOTH scopes, to
...
func TestModuleDoesNotSeeTheCallersVariables(t *testing.T) {
	...
	if _, ok := scope.Variable("environment"); !ok {
		t.Error("a module must see `environment`: it is a fact about the invocation, not the " +
			"caller's configuration, and a --var cannot even change it")
	}
}
...
`,
	...
	// Reported AND resolved to a poison value, so that every expression reading
	// it does not ALSO report "undefined variable".
...
// The declared kind is the whole point: it makes
	...
	for _, d := range ds {
		if strings.Contains(d.Summary, `variable "replicas"`) {
			t.Errorf("the diagnostic calls it a variable (%q). The user wrote an attribute on a "+
				"module call; naming the wrong construct fails the rule that a diagnostic names "+
				"what the user wrote, at the last hop, "+
				"after every stage upstream got it right", d.Summary)
		}
		// A module input literal carries Scope=unset, and this validator renders
		// ScopeLabel into a sentence. Something has to stop "The value supplied by
		// unset is a string" — a clause naming the ABSENCE of provenance as though
		// it were a source the user could act on.
...
// TestAWholeResourceReferenceAsAModuleInputIsRefused. A module input declares a
// TYPE, not a relationship, so there is no attribute declaration to project a
// bare ${net} against the way stage 6's bindAttribute does. Handing the
// unresolved, empty-attribute expression straight to expressions.Evaluate
// returns an Unknown carrying it and no diagnostic — the module's input, and
// everything inside it built from that input, would be created with the
// attribute silently unset.
