...
// TestFetchCommitPeelsAnAnnotatedTag. An annotated tag is an object in its own
// right, and `git ls-remote <url>
...
// TestFetchCommitResolvesAnAbbreviatedHash: an abbreviated hash cannot be
// fetched as a ref at all, so the depth-1 route does not work and fetchCommit
// must take the full-fetch-then-resolve route. A hash pin is spelled
// abbreviated in the documentation, so this is the form users write.
...
// TestLsRemoteCommitReportsAMissingRefRatherThanSucceeding: `git ls-remote` for
// a ref that does not exist EXITS 0 with empty output.
...
// the cheap path that detects a moved tag WITHOUT fetching. If this one
// regresses, every annotated-tag module reports a moved tag on every run.
...
// TestGitRefusesATransportTheLocationDoesNotName. Git applies
// url.<base>.insteadOf BEFORE choosing a transport, so a source that Parse
// approved as https:// can arrive at the transport layer as something else
// entirely — here, a local path. The per-invocation protocol policy is what
// refuses it.
//
...
// TestFetchDoesNotHangWhenTheRemoteAsksForAPassword is the end-to-end half. The
// server is a loopback httptest server returning 401, so
...
//
// What this test does NOT prove: deleting the GIT_ASKPASS override in gitEnv
// does not make it fail. gitEnv builds cmd.Env
...
// so the terminal-prompt half has no end-to-end failure mode under `go test`.
// This test does have one: delete the line and it fails.
