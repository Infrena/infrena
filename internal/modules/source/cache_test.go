...
func TestResolveReportsAMissingPathSource(t *testing.T) {
	...
	ds.Render(&sb)
	for _, want := range []string{"./modules/nope", "infrena.yml:3:5", "Suggested action:"} {
...
func TestResolveFetchesAGitSourceIntoTheCache(t *testing.T) {
	...
	if !strings.HasPrefix(got.Dir, filepath.Join(project, ".infrena", "modules")) {
		t.Errorf("Dir = %q, want it under the project's .infrena/modules", got.Dir)
	}
...
// opposite expectation, and it is what makes moved-tag detection possible. A
...
// one. Whether the CHANGE is reported to the user belongs to the modules.lock
// layer; here it is only that the checkout follows.
...
// TestPrepopulateCacheProducesAnEntryResolveAccepts.
//
...
// TestResolveWritesNoLockfile. Resolve fetches; it does not record. Everything in this test is a real
