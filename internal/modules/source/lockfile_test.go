...
// TestCheckIsAPureRead. `infrena validate` answers a question about a project
// and must not mutate it, and the only thing
...
func TestCheckIsAPureRead(t *testing.T) {
	...
	for _, e := range entries {
		t.Errorf("Check created %q; it must write nothing, or `infrena validate` becomes a command that mutates the project it is reporting on", e.Name())
	}
}
...
// TestCheckReportsAMovedTagAndNeverAdoptsIt is the whole point of the lockfile.
// One that silently adopts whatever it finds records history and enforces
// nothing — the appearance of pinning
...
func TestCheckReportsAMovedTagAndNeverAdoptsIt(t *testing.T) {
	...
	ds.Render(&sb)
	got := sb.String()
	for _, want := range []string{"v1.0.0", was, now, "modules.lock", "infrena.yml:3:5", "Suggested action:"} {
		if !strings.Contains(got, want) {
			t.Errorf("diagnostic does not mention %q — a report that does not name both commits cannot be acted on:\n%s", want, got)
		}
	}
	// The action must only name commands that exist: there is no `modules
	// update` subcommand and no --upgrade flag.
	if strings.Contains(got, "infrena modules") || strings.Contains(got, "--upgrade") {
		t.Errorf("the action names a command that does not exist:\n%s", got)
	}
...
// TestBumpingAPinIsANewEntryNotAMismatch is the case that separates a lockfile
// from an obstruction.
//
...
//
// Keying on rec.Source instead of rec.Source + ref is the plausible mistake,
// because "one entry per module source" reads perfectly natural, and nothing
...
// TestWriteLockfileWritesTheWholeSetSorted. The write is one call with every
// resolution the walk produced, never one call per Resolve: a lockfile built up
// entry by entry is readable half-written by anything that looks. That problem
// is solved once in internal/state/lock.go and must not be reintroduced here.
...
// TestWriteLockfileWritesNothingForAProjectWithNoRemotes: a lock file listing
// nothing suggests pinning is happening when it is not.
