...
const (
	...
	// published by this code — an interrupted fetch, or something a user put
	// there — and is discarded rather than loaded as a module.
	markerName = ".infrena-module"
...
// <ProjectDir>/.infrena/modules if it is not already there.
type Cache struct {
	// ProjectDir is the project root: the cache lives at
	// <ProjectDir>/.infrena/modules and modules.lock beside infrena.yml.
	ProjectDir string
...
func (c *Cache) root() string { return filepath.Join(c.ProjectDir, ".infrena", "modules") }

// checkoutDir is where a git source lives once fetched. It is content addressed
// on the location so that two sources naming one repository share a fetch, with
// the ref below it so that two refs of one repository coexist.
...
// resolved. Only the expansion walk sees every resolution, including the nested
// ones a fetched module's own module.yml declares, and only a complete set can
// be written in one shot — a lockfile built one entry per call here would be
// readable half-written by anything that looked.
...
// tag is mutable and a cache that never re-checks one cannot see that the tag
// moved. ls-remote transfers no objects, so the cost is one round trip, not a
// fetch.
...
//
// The rename is what makes a partially fetched tree impossible to observe: a
// checkout is read without taking the lock, so a fetch that wrote into the final
// path directly would be visible half-done for as long as it ran.
...
// place. os.Link fails with EEXIST if the target exists, which is create-or-fail
// and therefore exclusivity; os.Rename would overwrite, so two processes would
// both believe they held the lock, both would remove the checkout directory, and
// one would delete the tree the other had just published while it was being
// read.
//
// Unlike an environment lock, this one expires by timing out rather than waiting
// forever. An environment lock guards a mutation of real infrastructure, where a
// wrong guess about staleness is unrecoverable; this one guards a directory that
