...
// Env is the environment being expanded for, plus every environment the
// configuration declares.
type Env struct {
	// Name is the environment being expanded for.
	Name string
	// Declared is every environment the configuration declares, carried so
	// `skip`/`only` can refuse a name nothing declares. It is empty for a
	// project that declares no environments at all, in which case no name
	// can be checked and none is refused.
	Declared []string
}
...
//
// Evaluated in the scope it is handed, which is what makes both positions work:
// a module call's is evaluated in the caller's scope before the module is
...
//
// A scalar and a list mean the same thing, and either may be an expression. An
// expression that cannot be resolved to known text is an error rather than an
// empty list: a filter that quietly matched nothing would include every
// resource in every environment, which is the direction that deploys things.
