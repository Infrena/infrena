...
// Stage 5 COLLECTS resolutions and never writes them.
//
...
func TestLocalPathsProduceNoResolutions(t *testing.T) {
	...
	// the same fact behind requiring a pin on remote sources and not on these.
	// Without this, the filter could be deleted and every path-only project
	// would write a lock file full of empty commits.
	decl, dir := fixture(t, map[string]string{
...
`,
	...
	// Name the full cycle, not one participant.
...
// The bound counts INSTANTIATIONS — module boundaries crossed to reach a
// resource — never loads. Every directory under the project root holding a
// module.yml is auto-loaded, so a project with more modules than MaxDepth
// sitting side by side is ordinary and must compile.
//
...
// from. Origin.Module is the only way an error message can, and one
...
// case for (a module has no project name), so it reliably fails.
...
`,
	...
	_, ds := Expand(decl, variables.Scope{}, nil, Env{Name: "dev"}, dir, paths{}, nil, nil, nil)
	if !ds.HasErrors() {
		t.Fatal("a module file declaring `project:` must be refused by the module decoder: a module has no project name")
	}
	var stamped bool
	for _, d := range ds {
