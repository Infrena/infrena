...
// The one exception to the collision rule.
...
`,
	...
	exp, ds := Expand(decl, variables.Scope{}, nil, Env{Name: "dev"}, dir, paths{}, nil, nil, nil)
	if ds.HasErrors() {
		t.Fatalf("explicit beating implicit is not a collision: %+v", ds)
	}
	...
	if len(exp.Instances) != 1 || exp.Instances[0].Decl.Name != "explicit" {
		t.Fatalf("instances = %+v, want the explicitly loaded module's resource — "+
			"explicit config always wins over an implicit default", exp.Instances)
	}
}
...
func TestDiscoverySkipsDotDirectories(t *testing.T) {
	decl, dir := fixture(t, map[string]string{
		...
		".infrena/modules/abc123/net/module.yml": "resources:\n  cached:\n    type: test.thing\n",
