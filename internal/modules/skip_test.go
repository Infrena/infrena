...
// Stage 5: resolve `skip`/`only` and MARK. A later stage does the dropping.
...
func declIn(t *testing.T, body string, extra map[string]string) (*config.ProjectDecl, string) {
	t.Helper()
	files := map[string]string{"infrena.yml": body}
...
// TestASkippedResourceIsMarkedNotDropped. A resource that vanished here would
// land on a user as "no such resource" for a name in front of them.
...
// TestAProjectWithNoEnvironmentsChecksNothing — the same concession
// environments.Resolve makes, applied here too. With nothing declared there is
// nothing for a name to be wrong against.
...
// the feature exists to allow, not a generalisation of it.
...
//
// The input matters. `only: ${var.nosuchvariable}` errors during EVALUATION,
// from the expression package, so it never reaches the unknown-value guard this
// test is for.
//
...
// TestACompositeModuleInputIsEvaluatedNotDropped.
//
// A module call's attributes are parsed into one expression tree EACH, and a
// composite has none of its own, so its leaves must be walked separately.
// Skipping that walk silently drops the attribute and the module's own default
// wins.
