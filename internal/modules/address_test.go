...
`,
	...
	got := addresses(exp)
	if strings.Join(got, ",") != strings.Join(want, ",") {
		t.Fatalf("addresses = %v, want %v — expansion must produce one deterministic order, and "+
...
func TestOriginNamesTheInstantiationAResourceCameFrom(t *testing.T) {
	...
	if strings.Join(inst.Decl.Origin.Module, ".") != "net1" {
		t.Errorf("Origin.Module = %v, want [net1] — after flattening this is the ONLY way an error "+
...
// The hazard here: `prod` is a resource NAME the user wrote, and after
...
// it — deleting the sort changes nothing and those tests still pass.
//
...
// when they reorder a list that is meant to be a set.
...
// dot, and stage 6 walks only resource references for edges. Without this edge
// the plan is CLEAN and the apply fails after the outer resource has already
// been created, which a plan-only assertion cannot catch — so this asserts the
// edge rather than the rendered plan.
