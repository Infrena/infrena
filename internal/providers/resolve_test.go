...
func TestAnInstancesConfigInterpolates(t *testing.T) {
	...
	for _, tc := range []struct{ role string }{{"arn:prod"}, {"arn:dev"}} {
		...
		got, _ := table["test"].Config["iam-role"].AsString()
		if got != tc.role {
...
`)
	...
	tags, ok := table["test"].Defaults["tags"].Raw.(map[string]value.Value)
	if !ok {
		t.Fatalf("tags is not a map: %#v", table["test"].Defaults["tags"].Raw)
	}
...
`)
	...
	if table["main"].Plugin != "test" || table["acct2"].Plugin != "test" {
...
func TestAnInstanceWithNoInterpolationIsUnchanged(t *testing.T) {
	...
	if got, _ := table["test"].Config["iam-role"].AsString(); got != "plain-value" {
