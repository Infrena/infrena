...
//
// It runs after variables, because an instance's configuration interpolates
// them, and before module expansion, because that stage needs to know which
// instance a resource belongs to.
package providers
...
type Instance struct {
	Name   string
	Plugin string
	// Config is handed to the plugin. It is separate from Defaults because one
	// configures the provider and the other defaults a resource's attributes.
	Config map[string]value.Value
	// Defaults are attribute defaults for every resource using this instance.
	Defaults map[string]value.Value
	// Default marks the instance a resource gets when it names none.
	Default bool
	Origin  value.Origin
}
...
//
// It runs after variables, which is the whole point: `iam-role: ${var.aws_role}`
// with the variable set per environment is how one project reaches a different
// account in production than in dev.
...
func resolveAll(
	...
	for _, k := range keys {
		...
		// A composite, through the shared leaf walk rather than a second one.
...
//
// A resource reference is refused: a provider's configuration is needed before
// any resource exists, so `iam-role: ${some_db.arn}` would have the provider
// create the thing its own credentials depend on. Checked on the parsed
// expression rather than by noticing an unknown afterwards, because "unknown
// value" reads as an engine bug while naming the reference says what cannot
// work and why.
func resolveOne(
