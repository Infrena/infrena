...
type ResolvedConfig struct {
	...
	// Protections are the environment's protections, already resolved down the
	// `extends` chain. Carried on the compiled result so that `apply` reads the
	// same answer the planner does; `destroy` never compiles and takes them from
	// the chain directly, which is the same resolution because
...
// always the one being applied: protections inherit down `extends`, and a refusal
// that says "production prevents this" to somebody running `apply prod-eu` sends
// them to the wrong file to change it.
...
type Options struct {
	...
	// must not mutate the thing it is answering about, while `apply` is already
	// permitted to change the project directory. A command that only reads leaves
	// it false and still gets the comparison, which is a pure read — that
	// separation is what lets validate report a moved tag.
	RecordLocks bool

	Environment string

	// Version is the running build, for a project's `infrena:` floor to be checked
	// against. Empty means a development build, which every floor exempts.
	//
	// An input rather than read from internal/version, so that compilation is a
	// pure function of what it is given — and so the wiring is testable at all: a
	// test binary reports a development version, which is exempt, so a Compile
	// that read the build version itself could never be checked for calling the
	// check.
	Version string
	...
	Ctx context.Context
	// Secrets resolves ${secret.NAME}. Nil supplies none, and a reference then
	// reports "not set" rather than reading as empty.
	//
	// A function rather than a map, and injected rather than read from os.Getenv
	// here, so that compilation stays a pure function of what it is given.
	...
	// Templates resolves ${template.NAME} and ${file.NAME}.
...
// state. The encoding is written by hand rather than via encoding/json so that
// what is and is not covered is explicit and cannot drift when a struct gains a
// field.
func (c ResolvedConfig) Hash() (string, error) {
	...
	for _, addr := range c.Addresses() {
		...
		// Redundant with bind.go's sortedAddresses, and kept deliberately: Hash
		// is a pure function of the ResolvedConfig it is handed, and not every
		// caller builds that through bind.
		sort.Strings(deps)
		for _, d := range deps {
			write("depends_on", d)
		}
...
func hashValue(v value.Value, write func(...string)) {
	...
	if !v.Known {
		// An unresolved value's identity is the expression that will produce it.
		// Without this, two configurations differing only in which resource an
		// attribute references hash identically, and the staleness check that
		// compares hashes would miss the difference.
