...
// deliberately so. Resolve answers "what beats what", which is half the feature;
// the other half is "who can see it", and that half lives in the seam between
// config.Load's walk, stage 4's per-directory runs, and stage 5's Scope.In.
...
// does not satisfy that, so infrena.yml would refuse to compile before any of
// these tests reached their assertion. Whether a directory ought to be able to
// satisfy a project-wide declaration is left open.
...
// TestADirectoryScopedVariableBeatsTheProjectWideOne: a directory saying
// something about its own resources is more specific than the project saying it
// about all of them.
...
// plan's `[default]` marks and `infrena plan`'s value annotations all read this
// one field.
