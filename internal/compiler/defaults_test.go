...
// The precedence ladder, at the one place it is decided:
//
...
// visibly produce a third thing — which is the failure this asserts against. The rule
// is replaced, not merged; `merge()` is how a user asks for the union. Merging
// silently would leave no way to REMOVE an inherited key.
...
// The LIFECYCLE half of `defaults:`. Every resource accepts prevent_destroy and retain,
// so the instance's block may supply either for all of them at once — "prevent_destroy
// in production, not in dev" is the case the feature exists for.
...
//
// The `defaults:` check must not live inside the branch that CONSTRUCTS an instance:
// every other test in this package registers an already-built provider, so putting it
// there switches the fail-closed rule off with nothing failing.
//
