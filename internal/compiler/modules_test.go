...
// TestCompileExpandsAModule, in one assertion: a module call becomes real
// resources under module-qualified addresses, the caller's input reaches them,
// and a reference written INSIDE the module resolves there.
...
`,
	...
	// Nothing is addressed `prod`: a call is expanded away entirely.
...
// TestTwoModulesEachDeclaringADbResolveSeparately. Keyed on a bare name, both
// `db`s would be one resource and one module's reference would silently resolve
// to the other's — a wrong plan, not an error.
...
// TestAReferenceToAnAttributeThatDoesNotExistIsRefused. Without the check,
// ${store.endpoint} on a fake.network validates clean, produces a clean plan,
...
// TestAnUnboundNameNamesBothPossibilities pins wording implemented at exactly
// one site and asserted nowhere else. "no such resource" reads perfectly well
// and would satisfy every other test in this package.
...
// the module-path sibling to bind_test.go's root-level empty-attribute test. A
// whole-resource reference passed THROUGH a module input takes a different
// evaluation path (modules.evaluateCall, stage 5), which stage 6's projectRefs
// never sees.
...
// TestAWholeResourceReferenceToAModuleCallIsRefusedNotMisprojected. A module
// call is not a resource with schema'd attributes, so projectRefs must not treat
// it like one. Otherwise `vpc_id: ${prod}` against a consuming attribute that
// DOES declare References lets Qualify decline to fold (there is no output named
// ""), then lets projectRefs write "id" onto the still-empty attribute anyway,
// producing the self-contradicting `module call "prod" has no output "id"` —
...
// is the module-call variant of the same rule: `cidr: ${prod}`, where cidr
// declares no References at all. Getting it wrong produces BOTH "passes a
// resource to an attribute" (wrong — prod is a module, not a resource) AND a
// second, unrelated `module call "prod" has no output ""`, which names an
// attribute nobody wrote and invites the equally wrong fix of adding "" to the
// module's outputs.
...
// the module-output sibling of the test above. A module publishing
// `outputs: {whole: {value: ${net}}}`, where net is a resource INSIDE the
// module, otherwise compiles clean: Qualify's BindsModule arm folds the
// still-empty-attribute reference into an OpLiteral at the caller, and
// value.Expr.References() does not descend into OpLiteral.Literal.Expr — so
// projectRefs, canonicaliseRefs and the whole per-reference walk in bind.go
// never see it, and a consuming attribute like vpc_id ends up unknown with an
// empty-attribute expression that stays deferred forever.
...
// TestAWholeResourceReferenceAsAModuleOutputIsRefusedInsideAFunctionCall is the
// nested case: ${lower(m.whole)} at the caller cannot save a bare ${net}
// published as the module's own output. The reference must be refused where the
// module PUBLISHES it, not left to whatever expression the caller wraps it in.
