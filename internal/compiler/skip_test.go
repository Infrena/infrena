...
// PLAN.md §6.2's sharpest rule: a reference to a SKIPPED resource says so.
//
// "No such resource" would send a reader hunting for a typo in a name that is
// right there in the file, three lines above the reference.
...
// TestASkippedResourceIsStillChecked is what the MARKING buys, and neither the
// plan nor §6.2 said so — a sabotage found it.
//
// The plan claimed that dropping a skipped resource in stage 5 would make the
// reference rule impossible. It would not: Scope.skipped carries the NAMES, and
// that is what the rule reads. So dropping early passed every test except the
// one asserting the marking exists, which is circular.
//
// What the marking actually buys is this: a skipped resource's own attributes
// are still BOUND, so a mistake inside a `production`-only resource is reported
// when you plan `dev`. Drop it in stage 5 and that mistake is invisible until
// someone plans production — which is the run where finding out is most
// expensive. This test is the non-circular reason the design is what it is.
