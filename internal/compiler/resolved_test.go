...
func TestHashIgnoresOrigin(t *testing.T) {
	...
	a := res("db", "fake.database", map[string]value.Value{
		"engine": value.String("postgres", value.SourceExplicit).WithOrigin(value.Origin{File: "infrena.yml", Line: 3}),
	})
	b := res("db", "fake.database", map[string]value.Value{
		"engine": value.String("postgres", value.SourceExplicit).WithOrigin(value.Origin{File: "infrena.yml", Line: 99}),
	})
...
func TestHashIncludesProvenance(t *testing.T) {
	...
	if ha == hb {
		t.Error("Hash must include provenance")
	}
}
...
func TestHashDistinguishesDifferentUnresolvedReferences(t *testing.T) {
	...
	hb, _ := b.Hash()
	if ha == hb {
		t.Error("two unresolved values referencing different resources must not hash alike — stale-plan detection would miss a changed dependency")
	}
}
...
// TestConfigHashSeesAVariableFeedingADeferredExpression. hashExpr folds an
// unresolved expression by op name, function, ref NAME and argument count. If
// the evaluator does not fold resolved parts into literals first, two
// configurations differing only in a --var that feeds a deferred expression
// hash identically — and the staleness refusal compares exactly this hash to
// decide whether a saved plan still describes the configuration.
...
// TestConfigHashSeesAVariableFeedingADeferredCall is the sibling of the test
// above for a built-in call rather than a bare concat. If evaluateCall's
// non-default deferral path defers the whole SOURCE expression, a resolved
// argument sitting alongside an unresolved one stays an OpVarRef in the hash —
// ref NAME only — which is the same blindness one level along.
...
// TestConfigHashSeesAVariableFeedingANestedDeferredCallInAConcat. residual()'s
// unknown branch must use the evaluated Value's own Expr, not the source arg:
// reusing the source discards the inner fold of a partially-resolved OpCall
// nested in an OpConcat, and the residual then names the variable without its
// value, leaving hashExpr blind to it again.
...
// TestHashIsIndependentOfDependencyOrder pins plan determinism at the Hash()
// level: DependsOn arrives from map iteration upstream, so Hash must canonicalise
// it rather than trusting its caller. Four dependencies, because two collide
// often enough that a single comparison proves nothing. There is a matching test
// for the upstream sort in bind_test.go; either one alone leaves one of the two
// sorts removable with nothing failing.
