...
// scopeFor builds the variable scope bindReferences requires, through the real
// resolvers rather than a hand-made map — a test that constructs its own scope
...
`)
	...
	// The variable is not marked sensitive here, so this asserts the mechanism
	// rather than the classification: the unknown must carry its expression so
	// the executor can finish it.
...
// TestBindResolvesInterpolationInsideAList. The direction that matters: a leaf
// nested inside a list must be RESOLVED, never passed through as raw "${...}"
// text that reaches a plan looking like a literal.
...
`)
	...
	// correctly and the apply fails waiting for a value nothing produced.
	var deps []string
...
func TestRecordEdgeKeepsEarliestOrigin(t *testing.T) {
	...
	recordEdge(edges, "network", value.Origin{File: "infrena.yml", Line: 10})
	recordEdge(edges, "network", value.Origin{File: "infrena.yml", Line: 3})
	recordEdge(edges, "network", value.Origin{File: "infrena.yml", Line: 20})
...
// randomised per run and one compile that happens to come out sorted proves
// nothing. Hence the loop, and four dependencies rather than two.
//
...
`)
	...
	// Exactly one diagnostic. The reference has to be tracked as handled once
	// projectRefs has reported on it, or the still-empty attribute flows into
	// the attribute-axis check below and draws a second, unrelated
	// "fake.vpc has no attribute \"\"".
	if len(ds) != 1 {
...
func TestNoEmptyAttributeReferenceEscapesStageSix(t *testing.T) {
	// The invariant the parser's own comment relies on. An empty attribute
...
//
// A configuration with no `modules:` block is exactly this, and Expand produces
// it — going through Expand here instead would make every one of these unit
// tests an integration test of stage 5.
...
// TestAnUnregisteredConsumingTypeSkipsProjectionSilently. In projectRefs,
// consuming == nil is true both when the attribute declares no reference AND
// when the consuming resource's own type is unregistered — a nil def yields a
// nil attribute either way. Without the distinction, an unregistered type
// reaching a whole-resource reference draws the "declares no reference"
// diagnostic and then a second, unrelated one from the reference target's own
// attribute-axis check ("fake.vpc has no attribute \"\"") once the misprojected
// empty attribute reaches it.
//
// An unregistered type must report NOTHING from this stage. It has no schema to
// check anything against, and stage 7 (bindSchemas) already reports the
// unregistered type itself; that diagnostic should stand alone, which is why
// bindReferences on its own must come back clean here. The test below proves
// the full pipeline still refuses the configuration.
...
// writes it — `cidr` standing for `CidrBlock`, with a whole plugin naming design
// resting on it.
//
// A lookup that is exact against the user's spelling reports "declares no reference"
// about an attribute that plainly declares one, while the canonical spelling works.
