...
// The optional `infrena:` version floor a project may state.
...
// TestCompileChecksTheFloorAndStopsBeforeAnythingElse is the WIRING: every test above
// calls checkRequiredVersion directly, so deleting its call from Compile would break
// nothing at all.
//
...
//
// With a VCS remote present, debug.ReadBuildInfo reports a PSEUDO-VERSION that parses
// as 0.0.0, so without this every project stating a floor refuses a developer's own
// build. internal/version no longer returns one; this is the second line of defence,
// because a CI stamping "0.0.0" arrives by a different route.
//
...
//
// The test above covers a pseudo-version that parses to 0.0.0, which only happens
// before a repository has a release tag. Once one exists, the same `go build` reports
// something like `0.4.1-0.20260914210715-b41497c99237+dirty` — not zero, because a
// pseudo-version bases itself on the nearest reachable tag plus one patch. That slips
// straight past an isZero check and refuses a developer's own build.
