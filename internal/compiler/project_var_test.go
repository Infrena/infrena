...
// `project` is a process variable: a resource name or a tag almost always wants
// the project name, and threading it through as an ordinary variable would make
// every project declare the same line in its own variables.yml.

// TestProjectIsAProcessVariable — it resolves alongside the other process
// variables.
...
`)
	...
	// through a concatenated value. The test below asserts it on a bare
	// interpolation, which is where the rung is observable.
}
...
// process variables and nothing else.
//
...
//
// Refused rather than silently overwritten: Resolve would otherwise apply the
// entry like any other variable and seedProcessVariables overwrite it moments
// later with no diagnostic at all.
