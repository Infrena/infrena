...
// The hazard at the level a user meets it: a stream with no plan line is an
// error naming the problem, never an empty plan that applies nothing.
...
// The second guard: a `plan` line is accepted only from a file whose meta line
// says it reports
