...
// TestApplyOutputReportsFailedEventAndResult is the failing-apply case. It
// checks the failed event carries a non-empty error string, not merely that
// event=="failed" appears, and that result.error is PRESENT for a genuinely
// failed run — the opposite assertion from the errChanges case above. The two
// together discriminate the "error folded in unless errChanges" rule in both
// directions.
...
// TestRefreshOutputRedactsSensitiveDrift plants a sensitive attribute, drifts
// it outside infrena, and checks BOTH that the raw --output bytes never contain
// either cleartext value anywhere in the file — strong enough to catch a leak
// in a field this test does not otherwise inspect — AND that the "password"
// change is reported with both sides equal to the redacted marker. Checking
// only the absence of the secret would pass even if the attribute were dropped
...
// TestRefreshOutputCountsUnchangedRatherThanListingThem pins
// RefreshResult.Unchanged as a COUNT, not a list of addresses — the observation
// stream already named every one of them as it ran. A buildRefreshResult that
// appended addresses to a []string would produce a JSON array, which the type
// assertion to float64 fails against directly: the wire shape itself regressed,
