...
// seedState writes st directly to the backend. It takes the environment lock
// around the write because state.Local.Put refuses to write without one.
...
`)
	...
	// Hold dev's lock as another process for the whole test. Checking that
	// no lock file remains after Execute cannot distinguish "apply never
	// locked" from "apply locked, then released": both end with no lock file
	// on disk. If the no-changes path ever called backend.Lock, that call
	// would fail against this existing holder and Execute would error.
	backend := localBackend(dir)
...
`)
	...
	// The exact format comes from executor.Render
	// (internal/executor/summary.go), not from any wording chosen here.
...
// --auto-approve case: the run refuses BEFORE the prompt, with errNoApproval
// and exit 77, naming an escape that works, rather than telling a pipeline to
// type "yes". An empty reader, as opposed to one containing "no\n", is what
// reaches EOF on the first read.
...
// TestApplyOrdersDependentResourcesAndAppliesBoth is the only test in this
// file with a real dependency. The rest use a single resource, which cannot
// tell a correct BuildExecution(p, dependentsOf(p)) wiring apart from one
// that passed dependencies instead of dependents: with nothing to order,
// execution "works" either way. database.network references network.id, so
// this also exercises resolveAfter's cross-resource resolution
// (internal/executor/resolve.go) through the full CLI pipeline.
...
// TestApplyReportsFailureAndReturnsPlainError covers the one path no other
// test in this file reaches: a provider call that fails during execution.
// It pins that apply.go's own failure gate, and not errChanges, decides what
// is returned, and that the lock is still released on a failed run.
...
// TestApplyRePlansInsideTheLockNotJustBeforeIt pins that apply computes the
// plan twice: once unlocked, to show it and take approval, and once again
// immediately after the lock is acquired, so execution never runs against
// state or provider reality read before the lock was held. An apply that
// collapsed the two into one computePlan call would still pass every other
// test in this file, because nothing else here changes between the calls.
//
// The fake provider's failure injection is the seam. refresh.Refresh calls
// provider.Read exactly once per resource already in state, and computePlan
// calls Refresh once per invocation; network is already in state and matches
// the fake cloud, so a FailureRule with Nth: 2 fires on exactly the second
// read of network — the in-lock re-plan — and never on the unlocked preview.
...
// call, then behaves exactly like body. It stands in for a concurrent actor:
// bufio.Scanner.Scan in confirm calls Read at the exact moment a human's
// approval keystroke would arrive — after the unlocked preview, before
// backend.Lock — which is where a real race with another process would land.
...
// TestApplyPlanConvergesToNoChangesInsideTheLock reaches the post-lock
// "!p2.HasChanges()" branch no other test in this file takes, because every
// other test keeps provider reality identical across apply's two computePlan
// calls. It constructs deterministically the race re-planning inside the lock
// exists to guard against: mutatingReader fixes the fake cloud's drift — the
// drift the first, unlocked plan proposed to correct by replacing — the
// moment "yes" is read, standing in for another process having applied the
// same fix out of band. The in-lock plan then finds nothing left to do.
...
// needsDesired (internal/executor/apply.go), which excludes a replace's
// DESTROY phase from resolving desired values.
//
// Forcing needsDesired to `return true` leaves the rest of the suite green:
// an ordinary destroy or replace resolves its references against state. The
// shape that breaks is a replace whose NEW attributes point at a resource
// created later in the same run. The destroy phase runs first, finds
// ${network2.id} still unknown, and the run dies — taking the create phase
// and network2 with it as skips. The guard stops a destroy from demanding
// values only the create half needs.
//
// Seeded through two real applies rather than a hand-built plan, because what
// is under test is that the PLANNER produces this node shape and the executor
// survives it.
...
`)
	...
	if err := os.WriteFile(filepath.Join(dir, "infrena.yml"), []byte(`
project: myapp
resources:
  network:
    type: fake.network
...
// TestExecutorOptionsDoesNotConflateTheTwoConcurrencyBounds pins the
// per-provider concurrency bound at the seam where it was lost. The mechanism
// is tested in internal/executor; what broke was the WIRING. Both commands
// passed PerProvider: opts.Parallelism, which can never bind — the global
// semaphore admits at most Parallelism operations, so the per-provider check
// is unreachable. A bound that cannot fire is not a bound.
//
// It asserts against executorOptions, the function production actually calls,
// rather than reading the constant back: the constant being 8 is not the
// property, the two bounds being independent is.
func TestExecutorOptionsDoesNotConflateTheTwoConcurrencyBounds(t *testing.T) {
	...
	// must not TRACK Parallelism. Asserting inequality at a single value would
	// fail the moment the constant happened to coincide with that value.
	var seen int
...
// (destroy_test.go): recording dependencies on CREATE alone left destroy
// ordering correct for resources created afterwards, but not for resources
// whose dependencies later change.
//
// `db` is applied depending on `net`; a new resource `other` is added and `db`
// is made to depend on it too. Before the planner diffed dependency edges that
// planned as "1 to create" — only `other` — so db's recorded Dependencies
// stayed [net], the destroy edge for `other` never existed, and a later
// destroy could delete `other` first and strand `db`. Configuration's metadata
// must be able to catch up with what state records, or the recorded copy is
// authoritative in a direction nobody chose.
...
`), 0o644); err != nil {
	...
	if !strings.Contains(planOut.String(), "No changes.") {
		t.Errorf("the dependency update did not converge; a re-plan after apply must propose no changes:\n%s", planOut.String())
	}
}
...
// detecting end of input by peeking a byte off stdin introduced: the peek ran
// before the prompt was written, and on a terminal a read blocks, so the user
// saw a blank screen and the prompt appeared only after their keystrokes.
// Nothing may read stdin until the question has reached stdout.
...
// TestComputePlanNeverFoldsObservationsIntoState guards the one refactor that
// would turn a stale-`current` bug into an invisible one.
//
// The tidy-looking move, once the executor needs observations, is to write
// them into state the moment refresh returns — "one current truth, everything
// downstream reads it." It is wrong, and quietly so. st is two things at once:
// the left-hand side of the planner's diff, and the document the executor
// PERSISTS as it applies. The planner's Lifecycle and Dependencies diffs read
// st deliberately, since no provider owns those fields, and would start
// comparing configuration against whatever a provider's Read returned; the
// executor would then persist drifted attribute values as though they had been
// applied, with no provider call having made them true.
//
// So after computePlan, the plan must have been computed against what was
// OBSERVED, and st must still hold what was last PERSISTED. The merge belongs
// after planning, inside the executor — see executor.currentFor.
