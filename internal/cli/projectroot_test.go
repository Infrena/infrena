...
func TestFindProjectRootPrefersTheClosestProject(t *testing.T) {
	...
	dir := t.TempDir()
	write(t, filepath.Join(dir, "infrena.yml"), "project: outer\n")
	write(t, filepath.Join(dir, "infrena", "infrena.yml"), "project: inner\n")
...
func TestFindProjectRootDescendsIntoInfrena(t *testing.T) {
	dir := t.TempDir()
	write(t, filepath.Join(dir, "infrena", "infrena.yml"), "project: p\n")
...
func TestFindProjectRootDoesNotWalkUp(t *testing.T) {
	dir := t.TempDir()
	write(t, filepath.Join(dir, "infrena.yml"), "project: p\n")
...
// An error says what was expected and what to do.
func TestFindProjectRootNamesBothPlacesItLooked(t *testing.T) {
	...
	for _, want := range []string{"infrena.yml", "infrena/infrena.yml", "infrena init"} {
