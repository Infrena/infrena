...
// newDiscoverCommand builds `infrena discover [type...]`.
//
// Read-only: nothing is written, nothing is adopted into state, and no
// configuration is generated. It answers "what is out there" so that `import`
...
func newDiscoverCommand(opts *GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// Before any provider is asked, so a malformed --tag or --name
			// does not cost a minute of API calls first.
			...
			// This is the command that waits longest with the least to show
			// for it: one Discover call per instance over every type it
			// serves, which is minutes against a real account.
			progress := newProgressRenderer(cmd.OutOrStdout(), time.Now)
			defer progress.Stop()

			found, problems := discovery.Walk(cmd.Context(), reg, args, readRetryPolicy(), progress.Discovery)
			// Stopped before the table is printed, so a heartbeat cannot land
			// in the middle of it — the same ordering openRun's close function
			// keeps for every other command.
			progress.Stop()
			...
			// After Walk, which is after naming. Uniquifying a name is
			// order-dependent, so a filter applied inside the walk would
			// change the names of the resources that survive it.
...
// with the resources whose Name tag is empty looks like an empty account.
...
// renderDiscovered prints the survey table.
//
...
//
// The footer states both counts either way: a survey that quietly dropped fifty
// rows reads as an account with fifty fewer resources in it.
...
//
// The column holds the plugin's reason, not the flag: a row reading "system
// owned" tells a user a resource is special and nothing about why, and a flag
// overridden without being understood may as well not be there.
...
// reader with nowhere to look, and looking is the point of the column.
...
// something was: a suggested action that would show nothing new is not worth
// printing.
