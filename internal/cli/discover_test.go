...
// TestDiscoverShowsTheNameEachResourceWouldGet. The table exists so a user
...
// the engine cannot decide that a VPC is a default VPC without learning about
// AWS, so the plugin is the only party that can say it.
