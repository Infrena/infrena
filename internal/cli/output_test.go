...
// init: a shipped infrena carries no provider, and this package's TestMain
// injects one for every other test.
...
func stateExists(t *testing.T, dir, environment string) bool {
	t.Helper()
	_, err := os.Stat(filepath.Join(dir, ".infrena", "state", environment+".json"))
	return err == nil
}
