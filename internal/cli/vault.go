...
//
// A vault feeds ${secret.NAME}, the same reference the process environment
// feeds. Configuration never says where a secret came from, so moving one
// between a vault and a pipeline's environment is not a configuration change.
...
func newVaultEncryptCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Refused rather than nested. Encrypting twice produces a file
			// needing two passphrases in the right order, and the outer layer
			// hides that the inner one is there.
...
func newVaultDecryptCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// 0600: the file still holds credentials and has just stopped
			// being encrypted, so the one protection left is who can read it.
...
func newVaultViewCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Unredacted, deliberately: nothing here goes through
			// report.Format, because printing the contents is the request.
...
func newVaultCreateCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Before the editor, so a mistyped confirmation costs nothing.
			// Asked afterwards, it would throw away a file of secrets the user
			// had just written.
			passphrase, err := vaultNewPassphrase(opts)
			if err != nil {
				return err
...
func newVaultEditCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			path := projectPath(opts, args[0])
			// The passphrase is reused rather than asked for again: the
			// decrypt already proved it.
...
func newVaultRekeyCommand(opts *GlobalOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Rekeying is often done in response to a leak, and on its own it
			// is the wrong remedy: the old ciphertext is still in history and
...
// projectPath resolves a vault file argument against the project directory, not
// the process directory, because that is where the loader looks for it.
// Resolving against the shell's directory instead would let
// `infrena --chdir infrena vault edit secrets.yml` edit ./secrets.yml while
// every plan read infrena/secrets.yml, with no sign anything was wrong.
//
...
// openVaultWith is openVault, also returning the passphrase that worked. It
// exists for `edit`, which re-seals what it just opened: prompting a second
// time is a second chance to mistype, which would seal the file under a
...
// writeVault writes a vault file at 0600. The content is encrypted, but a
// world-readable ciphertext hands anyone on the machine unlimited offline
// guesses at the passphrase.
...
// temporary directory at 0700, so an editor's swap file, backup or crash
// recovery lands there too rather than beside the configuration where a
// `git add .` would sweep it up. The directory is removed on every path out,
