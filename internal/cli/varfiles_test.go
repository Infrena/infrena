...
// opts.VarFiles is loaded and threaded into compiler.Options.FileVars, not just
// that loadVarFiles works in isolation. The assertion names the exact value
// rather than a substring, because a short substring can appear in unrelated
// fixture data and pass with the feature disabled entirely.
...
// claim through the real `infrena plan` command rather than calling
...
// TestPlanAnnotatesVarFileAndVarWithTheirOwnSource is the end-to-end proof no
// unit-level test can give.
//
// An earlier attempt made the CLI-override annotation name its own source by
// reusing Value.Origin, and every test written for it — constructing a Value
// directly and rendering it — passed while the binary still regressed. A real
// plan reaches its resource attributes through "${var.cidr}" interpolation, and
// internal/expressions/eval.go's OpVarRef case re-origins every such reference
// to the referencing expression's site BEFORE the value reaches the renderer,
// which a test that never evaluates an expression cannot see. Value.SuppliedBy
// exists because WithOrigin's overwrite cannot touch it, and that claim is only
// worth a test that runs the real path: compiled configuration containing
// "${var.cidr}", through variables.Resolve and expressions.Evaluate, into a
// rendered plan.
...
`
	...
	t.Run("--var-file alone names the file as typed", func(t *testing.T) {
		...
		// The exact defect this guards: naming the DECLARATION file (infrena.yml)
		// instead of the file that actually supplied the value.
		if strings.Contains(out, "infrena.yml]") {
...
// produce a WARNING (a --var-file key shadowing one of infrena.yml's own block
...
resources:
  network:
    ...
    cidr: 10.20.0.0/16
`)
	// "resources" collides with one of infrena.yml's own top-level blocks —
