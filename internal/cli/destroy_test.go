...
func TestDestroyWithCorrectConfirmationRemovesEverything(t *testing.T) {
	...
	// A typed confirmation is only as good as what it makes the person read
	...
	// The plan header must name the real project, taken from state. Destroy
	// never calls config.Load or compiler.Compile, so state is the only source
	// for it, and getting it wrong would mislead the person about to confirm a
	// destroy against the wrong project.
	if !strings.Contains(stdout.String(), `Plan for project "myapp"`) {
		t.Errorf("stdout does not name the project from state:\n%s", stdout.String())
	}
	// executor.Render is the one shared result renderer for apply and destroy,
	// so it reports "Apply complete" whichever command ran. What differs is the
	// marker: a removed resource renders "-", not "+".
...
`)
	...
	// The apply summary must agree with the plan. It marks removals by finding
	// the address absent from state, and a forgotten resource is absent for the
	// same reason a destroyed one is — so this used to print "- network".
	// Result.Forgotten is what lets the summary say "=" instead.
	if !strings.Contains(stdout.String(), "= network") {
...
// TestDestroyRefusesWhenAPreventDestroyResourceIsPresent guards the most
// destructive command's most important protection. destroy.go contains no
// prevent_destroy logic at all, by design: the guard is entirely the
// planner's, reached through the same computePlan that plan and apply share.
// This pins the INTEGRATION of that delegation, not the guard's own logic,
// which internal/planner owns.
//
...
// every OTHER resource even while refusing the protected one, and
// planDiags.HasErrors() makes computePlan return before RunE reaches
// confirmation or backend.Lock — so the plan-time refusal must abort the
// ENTIRE destroy, not just skip the protected resource, and the plain
// resource must survive too. A destroy that "helpfully" destroyed everything
// except the guarded resource would still be a destroy nobody approved.
...
// TestDestroyMixOfPlainAndRetainedResourcesInOneRun covers what the
// single-resource tests cannot. TestDestroyWithCorrectConfirmationRemovesEverything
// only destroys one plain resource and
// TestDestroyForgetsARetainedResourceWithoutDeletingIt only forgets one
// retained resource, so neither proves that destroying ONE resource in a run
// leaves a RETAINED SIBLING in the same run alone. A bug that dropped the
// retain lifecycle whenever any other resource in the plan was a genuine
// destroy would pass both and only show up here.
...
// TestDestroyLockConflictNamesTheHolder pins that destroy actually takes the
// environment lock, not merely that no lock file survives it. The other tests
// in this file assert lock ABSENCE after destroy runs, which cannot tell
// "never locked" apart from "locked, then released". Pre-locking the
// environment as another holder closes that gap: if destroy never called
// backend.Lock at all, this would proceed to execute instead of failing on
// the contending holder.
...
// TestDestroyLockConflictNamesTheHolder, which only proves destroy REPORTS a
// lock another process stamped. Deleting destroy.go's
// state.WithOperation(ctx, "destroy") is invisible to every other test in
// this file, but it is user-facing: a second process hitting the lock while
// destroy holds it would be told the holder's operation is "unknown".
//
// The fake provider's cloud-file latency (LatencyMS) holds destroy's in-lock
// refresh Read long enough for a polling loop to Inspect the lock while
...
// TestDestroyPlanConvergesToNoChangesInsideTheLock reaches the post-lock
// "!p2.HasChanges()" branch no other test in this file takes, because every
// other test keeps state and provider reality identical across destroy's two
// computePlan calls.
//
// mutatingReader (apply_test.go) stands in for a concurrent actor: its before
// hook fires the moment confirm's scanner reads the typed environment name —
// after the unlocked preview, before backend.Lock is taken — and simulates
// another process finishing the same destroy out of band, deleting the
// resource from the fake cloud and overwriting the state file directly
// (bypassing state.Local.Put's lock check, which this process does not hold
// at that point, precisely because the prompt runs before the lock). The
// in-lock re-plan then finds nothing left in state.
...
// TestDestroyProjectNameComesFromAppliedState proves the project name against
// the real write path rather than a test shortcut. Every other test in this
// file that asserts a project name in destroy's plan header seeds state
// directly with state.New("myapp", …), which passes even if nothing in
// production ever stamps the project — and for a while nothing did, leaving
// those assertions green while guarding behaviour the binary did not have.
// Seeding through a real `infrena apply` makes the state destroy reads
// exactly what production would write.
...
// applyRealNetwork runs a real `infrena apply` end to end (through
...
//
// ResourceState.Dependencies is the ONLY source of destroy-ordering edges
// once a resource leaves configuration: the planner's dependentsOf reads
// state, not config, for OpDestroy and OpForget, because by then neither the
// resource nor some of its dependents are in configuration at all. Nothing
// ever wrote the field, so every state file carried an empty Dependencies and
// ordering silently failed for the one case where state is the only source —
// a teardown of resources already deleted from the YAML.
//
...
//
// `destroy --var` was once refused on the grounds that destroy compiles no
// configuration, so a variable had nothing to interpolate into. That is true of
// resources and false of `providers:` — an instance's own configuration
// interpolates variables, and destroy must construct that instance to send a
// delete to the right account, so refusing the flag made
// `region: ${var.aws_region}` destroyable by nothing.
//
// It asserts the absence of the old error rather than a successful destroy:
// what was wrong was the refusal happening before anything else ran.
