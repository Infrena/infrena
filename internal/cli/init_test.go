...
func TestScaffoldedEnvironmentsMatchTheVarsFiles(t *testing.T) {
	...
	body := read(t, filepath.Join(dir, "infrena", "infrena.yml"))
	for _, env := range []string{"production", "staging"} {
		if !strings.Contains(body, env+":") {
			t.Errorf("infrena.yml does not declare %s", env)
		}
	}
}
...
func TestInitTakesADirectoryArgument(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "myinfra", "infrena.yml")); err != nil {
...
func TestInitRefusesToOverwriteAndWritesNothing(t *testing.T) {
	dir := t.TempDir()
	runCommandIn(t, dir, "init")
	before := read(t, filepath.Join(dir, "infrena", "infrena.yml"))
	...
	if read(t, filepath.Join(dir, "infrena", "infrena.yml")) != before {
		t.Error("init modified infrena.yml before refusing")
	}
...
// TestInitRefusesWhenOnlyALaterFileExists, not just infrena.yml. The scaffold is
// written in sorted order and infrena.yml is not last, so a conflict on a file
...
func TestInitRefusesWhenOnlyALaterFileExists(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(target, "infrena.yml")); err == nil {
		t.Error("a refused init left infrena.yml behind: the existence check must run over EVERY " +
			"file before ANY is written, or a refusal half-scaffolds the directory")
	}
}

// TestInitScaffoldsAVersionFloor. The key turns "unknown key" into "this
// project needs infrena >= X; this is Y" for the NEXT break, which is the one
// nobody will remember to prepare for.
func TestInitScaffoldsAVersionFloor(t *testing.T) {
	...
	if !strings.Contains(read(t, filepath.Join(dir, "infrena.yml")), "infrena:") {
		t.Error("infrena.yml must declare a version floor")
	}
}
