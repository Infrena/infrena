...
//
// The formats matter more than the version number: a bug report that says
// "0.4.1" does not say whether that build can read the state file attached to
// it.
func newVersionCommand(opts *GlobalOptions) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --output writes the same thing as one JSON object, so a CI job
			// can assert on it rather than parsing the text.
			return writeVersionJSON(opts.Output, info)
		},
	}
}

// formats lists every versioned wire format, reading each number from the
// package that owns it so a bump cannot be made without this following.
//
// Assembled here rather than in internal/version, which may not import these:
// the compiler imports internal/version to check a project's `infrena:` floor,
// so a Formats() reading planner.PlanVersion would close an import cycle.
func formats() []version.Format {
	return []version.Format{
		{Name: "state", Versions: []int{state.CurrentVersion}},
		// A set, though only one today: the handshake negotiates, so a build
		// can keep talking to plugins written against an older protocol.
...
func writeVersionJSON(path string, info version.Info) error {
	...
	// 0600, matching the plan artifact and reports. Nothing secret is in here,
	// but one permission for everything this tool writes is a rule worth
	// keeping exception-free.
