...
// The interactive offer: when a plugin a project needs is missing, and a person
// is at the terminal, say what is published and offer to install it - then
// STOP.
//
...
// A binary that fails its checksum was reported as "the aws plugin is not
// available", with the suggested action being to install it - about a plugin
// sitting right there, already installed, whose problem is that it changed
// since. Same mistake the version-constraint case has its own branch for,
