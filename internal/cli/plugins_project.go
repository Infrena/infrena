...
//
// These decode and never compile, the same rule backendFor follows: a project
// with a broken resource must still be able to install the plugin that would
// fix it. They answer a name and a role only — which source may be installed
// from is still settled by searchSources and ensureTrusted.
...
// project here to ask.
//
// Both roles is an answer rather than an ambiguity: a project naming s3 as a
// provider and as a backend needs two binaries, so installing both is doing
// what was asked.
...
//
// Decode diagnostics are ignored on purpose: `providers:` and `backend:` are
// read off the parts that did decode, so a project with one unreadable resource
// can still say which plugins it needs — very often the plugin whose absence is
// why it does not read. A file that will not load at all is still an error,
// because answering "this project declares nothing" would send install off to
// guess.
...
// person types, and plugins.Kind already means something else in that package —
