...
func TestSchemasKeepsAnUntypedDeclarationThatHasADefault(t *testing.T) {
	// Schemas are optional. A declaration with a default and no type supplies a
	// value and constrains nothing.
...
func TestSchemasReportsEveryBadDeclarationNotJustTheFirst(t *testing.T) {
	// A declaration with a bad bound keeps its type and loses the bound rather
	// than stopping the walk, so a second bad declaration is still reported.
	// Every other bad-declaration test in this file calls Schemas with exactly
	// one bad declaration, so nothing else distinguishes "collect every problem"
	// from "stop at the first". Two declarations here, in two DIFFERENT failure
	// modes, so the test cannot pass by reporting the same problem twice: one
	// default has the wrong kind, the other is out of its own bounds.
...
func TestSchemasKeepsTheBoundsOrigin(t *testing.T) {
	// A range error must be able to say WHERE the bound was declared. Stage 2 put an Origin on the bound; losing it here would make
...
func TestSchemasDefaultDiagnosticNamesTheLineTheDefaultIsOn(t *testing.T) {
	// The check runs in stage 4 and still gets its location: the Origin travels
	// with the datum because Default is a value.Value. This
...
func intSchema(t *testing.T, min, max int64) Schema {
	...
	// Min's. Putting WithOrigin on d.Min instead would let the test pass by
	// coincidence, since the offending value never violates the minimum.
...
// kind-mismatch diagnostic to value.ScopeLabel. TestValidateRejectsTheWrongKind
// above only ever supplies a value with no SuppliedBy set, so it can only
// observe the "--var" FALLBACK and cannot tell "ScopeLabel, falling back
// correctly" apart from "v.Scope.String() hardcoded". This one supplies
// SuppliedBy exactly as config.DecodeVariableFile stamps a --var-file entry, so
// only a real ScopeLabel call can produce the right text.
...
func TestValidateUsesTheGrammaticallyCorrectArticle(t *testing.T) {
	...
	// A version that always returned "a" passes every other assertion here,
	// since they only check that the kind NAME is present, not the word before
	// it.
...
func TestValidateEnforcesFloatBoundsInclusively(t *testing.T) {
	...
	// kind, so the float side needs its own boundary coverage, not
...
func TestValidatePointsAtWhereTheBoundWasDeclared(t *testing.T) {
	// An error says what was expected AND where. A bound
...
func TestValidateRejectsANumericValueWhoseDatumDoesNotMatchItsClaimedKind(t *testing.T) {
	// numericDatumMatchesKind exists because trusting Kind as a claim about Raw
	// leaks: elsewhere it has produced a false equality and a printed secret.
	// A Value claiming KindInt while holding a non-int64 Raw — state or a
	// provider is the realistic source, a test fixture stands in for it — must
	// be reported, not silently treated as valid. Mutating
	// numericDatumMatchesKind to `return true` passes the rest of this suite;
...
func TestValidateFailsClosedWhenABoundDoesNotMatchItsDeclaredKind(t *testing.T) {
	// Schema is an exported struct with exported fields, so anything can build
	// one without going through Schemas() — the tests in this file do exactly
	// that. A KindInt schema whose Min is hand-built as a KindFloat value leaves
	// compareBounds unable to read the bound, and answering "no violation" for a
	// pair it could not compare would silently ACCEPT a value below the minimum.
	// A validator that says "no problems" when it could not perform the
	// comparison is worse than one that errors: the caller has no way to tell
	// "checked and fine" from "could not check". It must fail closed.
...
func TestValidateIgnoresAnUnknownValue(t *testing.T) {
	// `infrena validate` with no environment selected resolves environment-scoped
	// variables to unknowns. An unknown carries its Kind but has no
...
// lossy-conversion diagnostic to value.ScopeLabel.
// TestVarFileBoundViolationNamesTheFileNotDashDashVar in resolve_test.go only
// exercises boundDiag, so reverting this site's ScopeLabel call alone breaks
...
func TestCoerceDiagnosticNamesTheVarFileNotDashDashVar(t *testing.T) {
	...
	// nowhere else in this package, so this fragment cannot be
...
func TestValidateChecksListAndMapByKindOnly(t *testing.T) {
	...
	if ds := s.Validate(mixed); ds.HasErrors() {
		t.Fatalf("there is no element-type syntax, so a list's elements are unconstrained: %+v", ds)
	}
...
// TestParseTextCoercesAnExactFloatLiteralToInt. A numeric literal coerces to
// the declared kind wherever it appears, exactly or not at all — a SUPPLIED
// value included, not only a declared default or a --var-file entry. "42.0" is
// an exact int conversion, so `--var size=42.0` against `type: integer` must be
// accepted the same way a --var-file entry of `size: 42.0` is.
...
func TestParseTextRefusesListAndMapVariables(t *testing.T) {
	for _, k := range []value.Kind{value.KindList, value.KindMap} {
		if _, ds := schemaFor(t, k).ParseText("a,b,c", value.Origin{File: "--var"}); !ds.HasErrors() {
			t.Errorf("--var cannot supply a %s: parsing one needs a mini-language, which the variable syntax deliberately does not have", k)
		}
	}
}
...
func TestTheSuppliedByClauseSurvivesWhereThereIsAScope(t *testing.T) {
	...
	// credit a --var-file value to --var.
	s := Schema{Name: "replicas", Kind: value.KindInt,
		Origin: value.Origin{File: "infrena.yml", Line: 3, Column: 5}}
