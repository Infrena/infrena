...
func TestLayersReportsACycleByName(t *testing.T) {
	...
	// whether or not the cycle's members are ever named. A name that cannot
	// appear by accident is what makes the assertion mean anything.
...
// structNode is a struct used by value, which is how the planner instantiates
...
func TestGraphAcceptsAStructValueType(t *testing.T) {
	...
	// Two nodes sharing a Name but differing in Phase are distinct nodes. That
	// is the shape a replace relies on: a destroy and a create at the same
	// address.
	if g.Cycle() != nil {
