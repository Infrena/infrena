...
func TestReadyDoesNotWaitForAnUnrelatedSlowNodeInTheSameLayer(t *testing.T) {
	...
	// finishes; Walk must not, since child's own predecessors say nothing about
	// slow. That gap is why Walk exists beside Layers: a ready queue of nodes
	// whose own predecessors have completed.
	g := New[testNode]()
...
func TestDoneTriggersReadinessRegardlessOfPredecessorCompletionOrder(t *testing.T) {
	// child depends on zeta and alpha. Finishing zeta — the alphabetically later
	// of the two — first must not trigger readiness; only finishing both does,
	// whatever order they complete in. An implementation that wrongly assumed
	// predecessors resolve in ID order would pass this the other way around,
	// which is why the out-of-alphabetical-order finish comes first.
...
// The Skip-then-Done corner, which the Skip-then-Skip and Done-then-Done tests
// leave uncovered: c depends on both a and b; a fails and is skipped, retiring
// c along with it, while b is still running. A real dependency graph reaches
// this sequence whenever a failed operation and a sibling still in flight share
// a dependent.
//
// What it uniquely catches is Skip decrementing the Walk's total left count
// only for the id it was called with and not for c as well: with that change
// Remaining() below is 1, not 0. It does not catch Done(b) re-dispatching c —
// Done's own "status[next] != statusUnstarted" guard stops that regardless.
...
func TestDoneOnAnUnknownIDPanics(t *testing.T) {
	// "ghost" was never added to the graph at all. Accepting it silently, via
	// Go's zero-value map defaults, decrements Remaining for a node that was
	// never there to begin with. Graph.Edge panics on the equivalent mistake —
	// naming an id that was never added — for the same reason: ids reaching a
	// Walk always come from the executor itself, never unvalidated input.
...
func TestDoneCalledTwiceDoesNotDoubleDispatchASharedSuccessor(t *testing.T) {
	// child depends on both a and b. Two independent completion reports for a —
	// a caller bug, since a genuine operation only finishes once — must not let
	// child's remaining-predecessor count get decremented twice. That would
	// dispatch child as ready with b never having completed, breaking the rule
	// that a resource never executes before its dependencies. The assertion that
	// matters is not that the second Done(a) panics: it is that Done(b), the
	// only legitimate second predecessor to finish, is still the one and only
	// call that reports child ready. If the panicking second Done(a) call had
	// reprocessed a's successors before panicking, child would already be
	// dispatched here and this final Done(b) would report nothing instead of
	// [child].
...
func TestSkipCalledTwiceOnTheSameIDPanics(t *testing.T) {
	// Distinct from TestSkipOnASharedDependentIsNotDoubleCounted, which calls
	// Skip once each on two different dispatched ids that happen to share a
	// downstream dependent — a legitimate pattern that must not panic. This
	// calls Skip twice on the same id, which is always a caller bug: a is
	// already skipped, not dispatched, by the time the second call happens.
	g := build(t, []string{"a", "b"}, [][2]string{{"a", "b"}})
	w, err := g.Walk()
	if err != nil {
		t.Fatalf("Walk: %v", err)
