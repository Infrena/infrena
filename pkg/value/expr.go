...
const (
	...
	// OpVarRef references a variable by name.
	OpVarRef
	// OpTemplateRef references a file under templates/, whose contents are
	// interpolated in the same grammar configuration uses.
	OpTemplateRef
	// OpFileRef references a file under templates/ and reads it verbatim. It
	// exists because shell scripts and user-data contain ${...} of their own,
	// which interpolation would otherwise eat.
	OpFileRef
	// OpSecretRef references a secret by name, resolved from the process
	// environment at compile time and always marked sensitive.
	OpSecretRef
...
// Reference names another resource's attribute — or, under OpVarRef, a variable
// by name.
//
// Target is an address.Address rather than a bare name because a reference and
// an address name the same thing and must agree about what that thing is. With a
// bare name, ${db.id} written inside a module and ${db.id} written at the root
// are indistinguishable, so once a module's resources are re-rooted to
// module.net.db a lookup keyed on "db" misses the module's — or, when two modules
// each declare a `db`, matches the wrong one and returns its attributes as the
// answer. That is a silently wrong plan, not an error.
//
...
// Qualification fills the path from the scope recorded for each instantiated
// resource, before evaluation. Carrying the field is only half of it — a module
// path that nothing ever fills leaves every reference scope-relative and two
// modules' same-named resources resolving to one, with a field present that
// makes it look handled.
//
// Under OpVarRef only Target.Name is meaningful. A variable has no module path:
// it is resolved in the scope the expression was written in, before any of this.
...
// instantiation, turning a scope-relative reference into an absolute one.
//
// It delegates to address.InModule rather than appending to Target.Module,
// because that function copies the slice first: appending in place would let one
// instantiation's re-rooting alias into another's.
...
func (e *Expr) String() string {
	...
	switch e.Op {
	case OpLiteral:
		// A resolved sensitive value can be folded into a literal inside an
		// otherwise-deferred expression. Rendering it verbatim would be a
		// second redaction path alongside Format's, so this defers to the same
		// marker rather than growing its own.
...
func (e *Expr) inner() string {
	...
	case OpLiteral:
		...
		// call argument must not render its value just because it is nested one
		// level deeper.
		if e.Literal.Sensitive {
			return Redacted
		}
