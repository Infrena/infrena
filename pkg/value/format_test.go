...
// TestFormatNeverRendersRawItCannotVerify pins that Format refuses to render a
// Value whose Raw does not match its Kind, in every arm of the Kind switch and
// not only in the composite ones. Kind is a claim about Raw, so a scalar arm
// that formats Raw on trust prints a sensitive composite in clear text.
