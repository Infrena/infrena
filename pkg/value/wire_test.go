...
//
// A round-trip test cannot catch a rename here: it marshals and unmarshals through the
// same table, so a renamed spelling is symmetric and invisible to it. What a rename
// breaks is every plan artifact already written, which a newer build would then decode
// as a different operation or refuse outright.
//
