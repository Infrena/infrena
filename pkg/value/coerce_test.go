...
// exactness rule Coerce enforces, which internal/config reuses on a variable's
// bounds and default and on a value resolved against a schema.
...
// TestCoerceLossyConversionsFail is the refusing direction: a conversion that
// would change the value is never applied, and the value comes back with its
// original Kind and Raw rather than a half-converted one — a caller that
// forgot to check ok must not silently receive a wrong value.
//
// The two cases are the two directions it can be lossy: a fractional part
// rounded away converting float to int, and an int64 above 2^53 that does not
// survive round-tripping through float64.
func TestCoerceLossyConversionsFail(t *testing.T) {
	...
	// round-half-to-even when converted to float64 and back, which is what
...
// Raw. A coercion that silently cleared Sensitive would put a secret into a
// plan's output with nothing marking it as one.
func TestCoercePreservesEveryOtherField(t *testing.T) {
	origin := Origin{File: "infrena.yml", Line: 7, Column: 3}
