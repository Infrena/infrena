...
func TestUnknownIsNeverEqual(t *testing.T) {
	...
	if u.Equal(String("x", SourceExplicit)) || u.Equal(Unknown(KindString, SourceComputed)) {
		t.Error("an unknown value can never be proven equal to anything")
	}
}
...
// whose Raw does not match their Kind: never equal, and never a panic.
//
...
func TestEqualFailsClosedOnMalformedValues(t *testing.T) {
	cases := []struct {
		...
		{
			// A discarded type assertion makes both of these look empty.
		...
		{
			// == on an uncomparable type panics.
...
func TestCoerceRetypesAnUnknown(t *testing.T) {
	...
	// datum to lose, so retyping one is exact by definition. Coerce must
	// answer before it reaches AsInt/AsFloat, which report ok=false for a
	// value that has no datum yet.
...
// TestValueInModuleRecursesIntoComposites pins per-leaf re-rooting. Provenance
// is per-leaf, so a map with one bad key produces a diagnostic pointing at that
// key's origin; an origin that stopped at the outer Value could not say which
// instantiation it came from, which is the whole point of stamping.
