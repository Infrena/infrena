...
//
// An unknown value carries the expression that will produce it, and that
// expression has to survive being written down: an unknown decoded with no
// expression is indistinguishable from a computed attribute, so the executor
// drops it and the apply reports success having left the attribute unset.
//
// A mirror struct rather than tags on Expr, following wireValue: the on-disk
// language is a versioned contract with its own migration path, and keeping it
// separate from the Go field names is what stops a rename in one being a format
// break in the other.

// exprOpWireNames is the frozen on-disk spelling of every ExprOp.
//
// By name, never by number: ExprOp is an iota, so a constant inserted into that
// list would silently reinterpret every expression ever written. Entries here
// change only alongside a format version bump.
...
// stepKindWireNames is the frozen on-disk spelling of every StepKind, by name
// for the same reason exprOpWireNames is.
...
func toWireExpr(e *Expr) (*wireExpr, error) {
	...
	// Only the field the operation actually uses is written. An OpConcat
	// carrying a zero Literal would otherwise serialise one, and a zero Value
	// has KindInvalid, which marshalling refuses outright.
	switch e.Op {
	case OpLiteral:
		lit := e.Literal
		w.Literal = &lit
