...
func TestHasSensitiveFindsALeafInsideAComposite(t *testing.T) {
	...
	// A composite it cannot inspect counts as sensitive. HasSensitive does not
	// trust Kind over Raw; it declines to certify either way, and the
	// conservative answer is what the caller needs — CarrySensitivity's early
	// return is this function, so a false here drops a propagated secret's flag
	// and prints it.
...
// CarrySensitivity's doc comment promises.
//
// src's Kind says map and its Raw is not one, so nothing can be matched leaf
// by leaf, and the documented answer is to mark dst sensitive whole. Returning
// dst untouched instead would print a propagated secret in clear whenever a
// provider sent one back in a shape the engine did not expect.
