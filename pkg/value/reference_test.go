...
// TestReferenceStringIncludesTheModulePath pins that a reference renders as
// the address it names, module path and all.
//
// An implementation that carries a module path but drops it when rendering is
// worse than one that carries none, because References() and ConfigHash both
// read a reference through String().
...
// TestReferencesDoesNotCollapseSameNamedResourcesInDifferentModules pins the
// dependency-graph consequence of the module path.
//
...
// Reference would yield one edge where two are required — a dependency edge
// lost to an expression that looks entirely ordinary.
