...
// TestAnExpressionSurvivesTheWire pins that a pending expression round-trips.
// A plan records work not yet done, so a saved plan containing
// `network: ${net.id}` must not decode to a bare unknown: that is
// indistinguishable from a computed attribute, so the executor drops it and
// the apply reports success with the attribute left unset.
//
// State is kept clean at the other end instead: internal/state refuses to
// encode a resource attribute that is unknown or carries an expression.
...
// wireReference specifically for Path, which is easy to drop silently in both
// directions. A saved plan containing ${vpc.tags.Name} that round-tripped to
// ${vpc.tags} would make apply resolve the whole tags map instead of the Name
// key, with no error.
//
// The fixture uses a mixed path — an index step then a key step — because one
// with only a single kind of step passes against a wire step that hard-codes
// StepKey and ignores Index, or the reverse.
...
// TestAKnownValueWritesNoExpressionKey pins that the expression field is
// additive. Every value in a state file, and every value a provider plugin is
// sent or returns, is known — so if a known value's bytes are unchanged, none
// of those formats changed.
