...
// TestEqualIgnoresSuppliedByForEveryScope pins that a --var and a --var-file
// entry supplying the same datum are the same input: Equal must not
// distinguish them by SuppliedBy any more than it does by Scope. Without this,
// a --var whose value matches what a --var-file already said would plan as a
// change forever.
...
// TestSuppliedByRoundTripsThroughJSON pins that SuppliedBy survives
// serialisation. Value marshals through an explicit wireValue struct
// (json.go), so SuppliedBy needs its own field and tag — it will not
// round-trip by accident, and a missing tag fails silently: the value reads
// back with SuppliedBy "", which folds straight into annotation()'s fallback
// rather than erroring, so a saved plan renders "from --var" for a value that
// said "from f.yml" before it was saved.
...
// TestAnnotatePrefersSuppliedByOnlyAtCLIOverride pins the scope restriction:
// SuppliedBy wins over Scope.String() only at ScopeCLIOverride, and is ignored
// at every other scope even when set. Generalising the preference would mean
// that the day another rung starts stamping it, a value from variables.yml
// renders "from variables.yml" instead of "from base config" (see
// annotation()'s doc comment). The last two cases are the restriction itself,
// not the happy path: they set SuppliedBy at a non-CLI-override scope and
