...
type wireValue struct {
	...
	// Expr is the expression that will produce an unknown value. It is
	// omitempty so a known value — which is every value in a state file, and
	// every value a provider plugin is sent or returns — serialises
...
//
// It is deliberately separate from Kind.String(). Those spellings are the
// configuration language; this table is the on-disk one. They are independent
// contracts with different consumers and different migration paths, and only a
// table nothing else consults can guarantee that renaming a spelling on either
// side does not invalidate every state file ever written. Entries here change
// only alongside a version bump and a migration.
