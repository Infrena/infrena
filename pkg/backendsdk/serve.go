...
//
// This file is pkg/pluginsdk/serve.go with the provider parts taken out: same
// loop, same malformed-line handling, same cookie check, same write. Every
// place the two differ says why.
package backendsdk
...
func Main(b backend.Backend) {
	...
	// fmt.Println anywhere in the backend — or in an SDK it uses to talk to
	// a cloud, which the author may not even know about — lands in the log
...
//
// Requests are answered one at a time, in the order they arrive, so the host
// must not pipeline: one request, one response, before the next.
...
//
// No goroutine per request and no write mutex, which is the one real structural
...
// file.
//
// Request.ID is still carried and echoed, so a reply that has slipped out of
// step is caught rather than read as the answer to a different call.
...
// colleague's apply finding an empty environment.
...
//
// Three outcomes, and the third is what makes the method addable at all. A
// backend implementing backend.Validator answers for itself. A backend that
// takes no configuration can still answer the one question it has — whether the
// block set keys it will ignore — which is worth keeping, because a key that
// silently does nothing is a user who believes their state is in one place
// while it is written to another. Anything else reports unsupported, which the
// host treats as "ask no further", not as a failure.
...
//
// backend.Backend does not require a Name(), unlike a provider plugin, because
// the name here is only ever a label in the handshake and in a diagnostic, and
