...
func TestDesiredRejectsUnknownAttributes(t *testing.T) {
	...
	if _, err := r.Desired(); err == nil {
		t.Fatal("Desired() must refuse unknown attributes: providers are never called with unresolved values")
	}
}
...
// Clone must copy inside composite attributes, not only at the top level: a
// fresh Attributes map whose values still pointed at the original's maps and
// slices would leave two states one write apart. Cloning before refresh exists
...
// A deep copy that quietly dropped or reshaped a nested value would satisfy
// the aliasing test above, so the two have to be read together.
