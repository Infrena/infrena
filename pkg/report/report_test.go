...
// executor.Options.OnEvent and refresh's OnObservation are both callable
// concurrently from multiple workers, so Writer must serialize its own access.
// Under -race a missing mutex trips the detector on its own, because
// bytes.Buffer is not safe for concurrent use; the assertions catch the
// separate bug a lock does not prevent, an interleaved partial write producing
// a line that is not valid JSON or a line count that does not match.
...
// "version" is the handshake field and must never be dropped, so this asserts
// it is present and correct rather than only that marshalling succeeded — which
// would pass even with Version omitted by a bad json tag.
...
// This fails if the package ever grows a second redaction path of its own
// instead of calling pkg/value.Format. One renderer, one place to get it right.
func TestFormatRedactsSensitiveValues(t *testing.T) {
	// Built by hand to keep the dependency on pkg/value narrow: this checks that
	// Format calls through, not that value.Format is itself correct.
...
func TestVersionIsFour(t *testing.T) {
	// Each new line kind raises this, so a consumer that only understands an
	// older version can tell. It is independent of every other format version.
	//
...
// The status crosses as a word. A consumer that had to map an exit code back to
// a meaning would be keeping a copy of a table that lives in the CLI.
...
// writes. That is the difference between the two kinds: one is a replayable
// input, this one is a report of a run, and no value reaches a report except
// through Format.
func TestWritePlanChangesRedactsValues(t *testing.T) {
	...
	// Not "plan": readSavedPlan scans a stream for that type and applies what it
	// finds, so a redacted plan carrying it would be applied as if its values
	// were real.
...
// A plan line promises a future value, so an unknown in it reads the way the
// plan renderer reads it. ReportFormatOptions' "(unknown)" is for a value still
// not known after a run, which is an anomaly. A UI rendering a plan line beside
// a terminal's rendering of the same plan must not see two words for one thing.
