...
// correctly is pkg/backendsdk's business, and testing it once per backend would
// test infrena's code rather than the author's. What differs between backends,
// and what silently corrupts state when it is wrong, is whether a second lock
// is actually refused and whether a write without one is.
//
// That every backend must lock is enforced at the type level: a backend without
// the methods does not compile. Nothing checks what those methods exclude. This
// does, because a third-party binary cannot be held to a doc comment.
//
...
// taking either is a suite nothing can prove.
//
// Self is the implementing type because Run's callback takes the concrete type:
// *testing.T's Run takes a func(*testing.T), so TB[*testing.T] is what
// *testing.T satisfies, and inference fills Self in for a caller that simply
// passes its own t.
...
// what keeps a rule broken by one check from being blamed on another; the store
// being shared between the values one check opens is what makes two infrena
// runs on two machines expressible at all, and remote state exists for no other
// reason.
//
// Most checks open one backend and are handed it. The one that cannot — a
// second run taking over a lock the first still believes it holds — opens two,
// and that is the only reason the factory has this shape rather than simply
// returning a backend.
//
...
// aSecondLockIsRefused is the rule the rest of the contract stands on: a lock
// that does not exclude lets two applies mutate one environment, and the
// loser's resources become state nothing records.
...
// aWriteWithoutALockIsRefused is that same exclusion enforced where it cannot
// be skipped. Refusing in the caller is caller discipline; refusing in the
// backend is a property of what the storage allows.
...
//
// The sequence is the one `infrena state unlock` creates. A holds the lock; an
// operator judges A stale and forces the lock off; B takes it and starts
// applying. A is still running, and its next Put lands on an environment B now
// owns — two applies mutating one environment. A backend that asks its own
// memory whether it holds the lock answers yes and writes, silently, over B.
//
// The local backend is exactly the shape this asks for: it re-reads the lock
// file before every Put rather than trusting what Lock handed back, and a
// store-shaped backend gets there by comparing the stored lock object with the
// one it wrote.
//
...
// and this returns silently rather than reporting it a second time.
...
// environment B owns. Two applies mutating one environment is the failure the
// contract exists to prevent, and the run that loses leaves resources nothing
// records.
//
...
// "do I hold this environment's lock?" — since after B locks, the answer to the
// wrong question is yes. That backend writes straight over B's apply with no
// error anywhere, and only a second instance over the same storage shows it.
//
...
// bug.
//
// It stands down twice, so that a backend already caught by a weaker rule fails
// one check rather than two: once if A can write with no lock at all
...
func aWriteAfterAnotherRunTookTheLockIsRefused[Self TB[Self]](t Self, open func() backend.Backend) {
	...
	// B does the forcing, because B is who runs `infrena state unlock`, and
...
// has never applied anything is what `infrena plan` asks about first, and a
...
// stores bytes and never parses them; one that re-encodes what it was handed is
// a second reader of state, and the difference shows up as a plan that
// disagrees with the last apply.
...
// holder is what `infrena state unlock` prints so they can go and check whether
// that run is still alive.
//
// A backend whose second lock succeeds is not this check's business — that is
// aSecondLockIsRefused, and reporting it twice would leave neither failure able
// to say which rule was broken.
