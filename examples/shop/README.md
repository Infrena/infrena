...
`provider:` — see [docs/configuration.md](../../docs/configuration.md). That is deliberately not
shown here: a second account is a
...
```
infrena.yml                     the project, its variable declarations, environments, modules
...
single `infrena.yml` plans identically, so move things around freely. Two rules are worth knowing
before you do:
...
**Drift.** Edit `.infrena/fake-cloud.json` after an apply — change the database's `size` to 999 —
then:
