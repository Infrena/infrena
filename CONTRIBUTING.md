...
**Open an issue first for anything non-trivial.** Infrena is designed rather than accreted, and
most decisions in the codebase were argued before they were written. A pull request that cuts
across one of those decisions is not a bad pull request, but it is a conversation, and it is
cheaper to have before the code exists.
...
See the `pkg/pluginsdk` package, which is the whole of a plugin's `main()`, and
[docs/provider-hazards.md](docs/provider-hazards.md) for the traps worth knowing first.
