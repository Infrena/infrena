...
//
// The split is what lets `cloud: ${var.path}` work. A provider object needs its
// cloud file; the path may interpolate a variable; resolving that variable needs
// a compile; and a compile needs the schemas. Handing the schemas over
// separately breaks the cycle. See provider.Plugin.
type Plugin struct {
...
//
// `cloud:` is this provider's stand-in for an account, so an instance that names
// no file gets its own, named after itself: two instances pointing at one file
// would be two names for one account. The implicit instance is the exception and
// keeps the unsuffixed path, so a project written before `providers:` existed
// finds the cloud it already has.
