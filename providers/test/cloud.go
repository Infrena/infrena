...
const DefaultCloudPath = ".infrena/fake-cloud.json"
...
type CloudResource struct {
	...
	// account. The declaration lives in the provider's own world because the
	// engine cannot work it out without knowing about the cloud in question,
	// which is why the plugin protocol carries the flag.
...
//
// A boolean cannot express the middle category, and all three get materially
// different executor behaviour: a Create is never retried on an ambiguous
// failure, a Delete only when the provider says it is safe. This provider is
// the only thing that can produce those errors on demand.
...
// Refresh reads every resource concurrently and the provider rewrites this file
// on every operation including Read, so a plain os.WriteFile would let a
// concurrent reader see a half-written file and report "unexpected end of JSON
// input", which looks like a provider failure rather than a harness bug.
