...
// A capability a provider does not offer must report so rather than returning an
// empty result: a discovery that silently finds nothing and one that cannot run
// look identical to a caller. The fake provider offers both Discover and Import,
// so that assertion belongs to whichever provider does not — pkg/provider's
// ErrNotImplemented is the contract for anything a provider leaves unbuilt.

// The regression guard for persisted failure bookkeeping: FailureRule's
// Seen/Fired fields must be exported and persisted, and begin() must save the
// cloud after every ShouldFail call, not only on failure. The provider reloads
// the cloud file on every operation — it must, to observe hand-edited drift — so
// if Seen is not written back after a non-firing check, it resets to zero on the
// next load and an Nth: 2 rule can never fire.
...
func TestUpdatePreservesDependencies(t *testing.T) {
	// The Dependencies recorded in state are the only source of destroy-ordering
	// edges for a resource that is no longer in configuration. An executor that
	// persists Update's return value — which "state is persisted after every
	// operation" implies — would erase those edges for every resource it ever
	// updates, and the failure would surface much later as a destroy in the
	// wrong order.
	p, _ := newTestProvider(t)
...
func TestReadPreservesCarriedFields(t *testing.T) {
	...
	// Lifecycle is carried for a sharper reason than the others: `infrena
	...
	// it. It is the one carried field whose loss is silent and destructive.
...
// The cloud file must hold plain JSON for a composite attribute.
//
...
// objects. That defeats the premise that a human can hand-edit fake
// infrastructure, and reading it back yields a Map whose every leaf is itself a
// four-key kind/known/raw/source Map — inexplicable permanent drift rather than
// an obvious serialisation bug.
...
// Drives ClassifyError from the cloud file, the way a person or an executor test
// would.
//
// A `Retryable bool` maps onto exactly two of the three provider.Retryability
// constants, and the three get materially different executor behaviour, so a
// boolean leaves a third of the retry logic untestable — this provider is the
// only thing that will ever produce these errors.
...
// A typo in a hand-edited file must stay loud. Silently treating "sfe" as
// not-safe is a value interpreted by its surface text, which is its own defect
// class.
...
func TestOperationsOverlapRatherThanSerialise(t *testing.T) {
	...
	// latency. If it covers the delay, every concurrency test against this
	...
	// Both sides are measured, which is the point.
	//
	// Comparing the concurrent time against a predicted serial time,
	// resources*delay, assumes the only thing four reads cost is four delays —
	// no file loading, no goroutine scheduling, no race detector, and a runner
	// that is not busy. On a shared CI runner none of that holds, and raising
	// the threshold each time it fails is a losing move, because the quantity
	// being guessed at is the one that varies.
	//
...
// Including resources this project never created, which is the entire point:
// discovery is for infrastructure that predates the tool. Neither fixture
// resource carries an `address`.
...
// Map iteration is randomised, so one run proving the order proves nothing.
...
// `infrena discover aws.rds` asks one question of a large account, and answering
// it by fetching everything and discarding most is how discovery becomes too slow
// to use.
...
// An account with nothing in it is not an error, and neither is a project that
// has never applied anything.
...
// A user importing by ID has usually mistyped it or is looking in the wrong
// account; an error that does not repeat the ID leaves them unable to tell which.
...
// `import fake.network db-9` names a real resource of the wrong type. Adopting it
// anyway writes state claiming a database is a network, and the next plan
// proposes replacing real infrastructure to fix a disagreement the tool invented.
