...
// The local -> S3 -> local round trip (spec §7, plan Task 5).
//
...
// TestStateSurvivesARoundTripThroughAnotherBackend is the whole of Task 5.
//
...
// `.infra/state/<environment>.json`, which this file names directly rather
// than asking infrena, for the reason it reads the bucket directly.
func localStatePath(dir, environment string) string {
	return filepath.Join(dir, ".infra", "state", environment+".json")
}
...
func buildS3Backend(t *testing.T) string {
	...
	// The BINARY NAME is how infrena finds it (§31.3): a backend is looked up
	// as infrena-backend-<name> for the `plugin:` the block names.
...
func installBackendBinary(t *testing.T, dir, built string) {
	t.Helper()
	plugins := filepath.Join(dir, ".infra", "plugins")
