...
//
// The hazard that disjunction closes: if only declared environments are
// reachable, removing one from configuration leaves `destroy` working while
// `plan` and `refresh` refuse with "unknown environment". The only reachable
// command is then the destructive one, run blind, which inverts plan-then-apply.
...
// removeSandbox rewrites infrena.yml without the sandbox environment.
func removeSandbox(t *testing.T, dir string) {
	t.Helper()
	p := filepath.Join(dir, "infrena.yml")
	b, err := os.ReadFile(p)
	if err != nil {
		t.Fatal(err)
	}
...
// TestPlanOnARemovedEnvironmentProposesDestroyingEverything.
//
...
// TestAnUnknownStatelessEnvironmentIsStillAnError is the half that keeps the
// rule a disjunction rather than "anything goes".
//
...
//
// The teardown has to complete: once state lists nothing, the "has state"
...
func TestApplyingARemovedEnvironmentTearsItDownAndStopsBeingReachable(t *testing.T) {
	...
	statePath := filepath.Join(dir, ".infrena", "state", "sandbox.json")
	...
	// Reachability, not the state FILE, which deliberately survives: reachability
	// keys on whether state lists resources, so an emptied environment is already
	// unreachable, and deleting the file would discard the serial that detects
	// stale plans.
...
// destroy is exactly what you want to see first, so refresh must reach a removed
// environment for the same reason plan does.
...
// Per-environment resource filters through the built binary: one configuration,
// four environments, four different resource sets, no variables involved beyond
...
// TestOneConfigurationFourEnvironments.
//
...
func TestOneConfigurationFourEnvironments(t *testing.T) {
	...
	for _, env := range []string{"dev", "staging", "sandbox", "production"} {
		...
		// The module's own filter, driven by an input the caller supplied.
	...
	for _, env := range []string{"dev", "staging", "sandbox", "production"} {
		b, err := os.ReadFile(filepath.Join(dir, ".infrena", "state", env+".json"))
		if err != nil {
...
// TestAddingSkipToAnAppliedResourceProposesDestroyingIt. Editing a filter is a
...
`, 1)
	...
	if err := os.WriteFile(filepath.Join(dir, "infrena.yml"), []byte(patched), 0o644); err != nil {
		t.Fatal(err)
	}
...
// TestAnEnvironmentTypeKeyFailsValidate. Environments have no `type:` key; an
// older design did, so the refusal has to point at the replacement.
