...
// TestTheReportDescribesItself.
//
// A report is archived and read later, often by somebody establishing what
// happened rather than replaying it. Two things it could not previously say
// about itself: which BUILD produced it — `version` says how to decode the
// lines, not what wrote them — and which state version the run produced.
//
// The serial is the strongest link a report can carry. Addresses say what was
// touched and timestamps say when, but a serial says which state this run MADE,
// so two reports can be ordered against each other, a gap shows a run nobody
// archived, and a report can be matched against a state file rather than
// believed on its own.
...
`)
	...
	stateBytes, err := os.ReadFile(filepath.Join(dir, ".infra", "state", "dev.json"))
	if err != nil {
