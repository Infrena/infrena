...
// TestASecretComesFromTheEnvironmentAndIsNeverPrinted.
//
// The environment is the only secret source this CLI reads, which is what keeps
// the guarantee simple: infrena never stores or transports the value.
...
// rather than a `{secret: NAME}` mapping: a connection string is the common
