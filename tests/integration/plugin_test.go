...
// THE REAL PLUGIN, not an in-process double.
//
// A shipped infrena carries no provider, so the binary this suite runs has none either.
// Every other suite in this repository registers the fake provider in process; this one
// builds `infrena-plugin-fake` from its own repository and puts it on the search path, so
// the path a USER takes is proved somewhere: a subprocess, a handshake, a cookie, stderr
// forwarding, and the host's trust rules over a real pipe.
//
...
func buildFakePlugin() {
	...
	// three is the directory holding it, whose sibling is the plugin.
	//
	...
	// private module, so resolving that require means an authenticated fetch;
	// supplied by the workspace, it is never fetched at all, and only ordinary
	// public dependencies go to the proxy.
	//
	// Declaring the pairing HERE rather than relying on a `replace` in the plugin's
	// own go.mod is deliberate: this suite states which infrena the plugin is built
	// against, instead of inheriting it from another repository where a change
	// nobody here reviews can silently repoint it. CI checks the plugin out at a
	// fixed path beside this repo for the same reason.
...
// TestAShippedBuildCarriesNoProvider is the only test that can make this claim:
// every in-process suite injects the fake double, so a build that accidentally
// regained a compiled-in provider would pass all of them.
//
// It runs the built binary WITHOUT --plugin-dir, which is what a user has before
// installing anything.
func TestAShippedBuildCarriesNoProvider(t *testing.T) {
	dir := t.TempDir()
	// The project is written here rather than scaffolded by `init`: the scaffold's
	// example resource is commented out precisely so a fresh project validates on a
	// machine with nothing installed, so it cannot be the thing that demands a
	// plugin. TestTheScaffoldValidatesWithNoProviderInstalled is that other half.
	body := []byte("project: shipped\nresources:\n  network:\n    type: fake.network\n    cidr: 10.0.0.0/16\n")
	if err := os.WriteFile(filepath.Join(dir, "infrena.yml"), body, 0o644); err != nil {
		t.Fatal(err)
	}
...
// the in-process init tests prove with a double: what `infrena init` writes must pass
// `infrena validate` on a machine that has installed nothing.
//
