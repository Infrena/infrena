...
//
// Discovery only PROPOSES a name, from a tag or the provider ID, and nothing
// else renames a managed resource — so without --as, adopting a VPC means
// living with `network-vpc-0a1b2c3d`.
//
// Import time is the one moment renaming is safe, which is why this is a flag
// and not a general `state mv`: nothing references the resource yet, so there
// is no dependency edge, no generated file and no other resource's ${...} to
// rewrite.
...
//
// The danger of this command is that it LOOKS like a deletion, and several
// import diagnostics suggest it, so the assertion that matters is the last
// one: the resource is still there afterwards.
