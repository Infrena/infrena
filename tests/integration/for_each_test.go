...
func TestRemovingOneEntryAffectsOnlyThatResource(t *testing.T) {
	...
	writeIn(t, dir, "infrena.yml", strings.Replace(zonedSubnets,
...
//
// for_each on a module call and for_each on a resource are honoured by
// different loops. When the module-call loop ignored it, a call carrying
// for_each expanded exactly once and produced a clean plan saying nothing: ask
// for two databases, get one.
...
`
	...
	writeIn(t, dir, "infrena.yml", strings.Replace(both,
...
//
// The generic "no such output" message must not be used here: the output exists,
// and a keyed call simply has no single set of outputs to answer from, so that
// message contradicts itself and sends the reader hunting for a typo.
...
//
// A guard that rejects any bracket in the first segment leaves only the
// whole-resource form ${subnet["alpha"]} working, and makes the form anybody
// reaches for first an error telling them to remove the bracket.
//
// It must go through the binary. The parser's own unit tests call the reference
// parser with segments already split, so they never traverse that guard.
...
// bucket name, a version. An address parser that splits on dots blindly turns
// subnet["eu.west.1a"] into three segments and refuses it.
//
// It must go through the SAVED PLAN. Plan-and-apply never reads an address back
// from text, so asserting only that the apply works passes against a build
// where `apply --plan` — the reviewed-plan route into a protected environment —
// fails after a human has already approved the plan.
