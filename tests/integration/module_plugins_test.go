...
// provider resource. Module files are read later, so the registry must be
// topped up with whatever expansion revealed; otherwise a provider used ONLY
// inside a module never loads, and the plan fails naming a type inside the
// module, blaming the module for the plugin's absence.
//
// This project declares no `providers:` block on purpose. Adding one is the
// workaround, and a block a reader gains nothing from is exactly what makes
// `providers:` optional in the first place.
...
//
// Loading plugins late must not turn a real absence into a worse message: the
...
// TestAProvidersBlockIsEnoughForAModuleOnlyProject. Naming a plugin explicitly
// must keep working, since plenty of projects carry the block anyway for a
// region or an account.
