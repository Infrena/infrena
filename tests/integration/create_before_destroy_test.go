...
// no moment when the resource does not exist (PLAN.md §38.2).
...
func TestCreateBeforeDestroyLeavesTheNewObjectInState(t *testing.T) {
	...
	writeIn(t, dir, "infra.yml", strings.Replace(cbdProject, "CIDR", "10.9.0.0/16", 1))
	if a := run(t, dir, "apply", "dev", "--auto-approve"); a.ExitCode != 2 {
		t.Fatalf("replace: %d\n%s", a.ExitCode, a.combined())
	}
...
//
// The planner already proposes the dependent's update, which is why no
// refusal is needed here: the concern recorded in §38.2 was that
// create_before_destroy would have to propagate to dependents the way
// Terraform's does, and it does not, because the dependent is updated rather
// than replaced.
...
`
	...
	writeIn(t, dir, "infra.yml", strings.Replace(withDependent, "CIDR", "10.9.0.0/16", 1))
...
`
	...
	writeIn(t, dir, "infra.yml", strings.Replace(plain, "CIDR", "10.9.0.0/16", 1))
	if a := run(t, dir, "apply", "dev", "--auto-approve"); a.ExitCode != 2 {
		t.Fatalf("replace: %d\n%s", a.ExitCode, a.combined())
	}
	if d := deposedCount(t, dir, "net"); d != 0 {
