...
func TestTheShopExampleTeachesTheLayout(t *testing.T) {
	...
	// Moving the scoped vars file to another resources directory must take
	// `size` out of scope where it is used. A build that made every scoped
	// variable global would still pass the plan above, because the right value
	// would appear anyway; only this catches it.
	moved := t.TempDir()
	...
	// What the module boundary refuses, both halves: a caller cannot pass what
	// the module does not declare, and a module cannot reach a caller's
	// variable to get around that.
	both := t.TempDir()
	copyTree(t, src, both)
...
func copyTree(t *testing.T, src, dst string) {
	...
	err := filepath.Walk(src, func(p string, info os.FileInfo, err error) error {
		...
		// Skip dotfiles: .infrena/ is state and a fake cloud left behind by
		// anyone who ran the example in place, and copying it makes this test
		// inherit a developer's local run.
