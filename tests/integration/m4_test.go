...
// TestValidatePlanAndApplyResolveTheSameVariableLayers: validate, plan and apply
// must resolve ${var.cidr} to the same value.
//
// It asserts behaviour, not structure. "They share a helper" is not the
// property — a test asserting all three call compilerOptions would pass even if
// compilerOptions itself were wrong.
...
`)
	...
	for _, cmd := range [][]string{
		...
		if r.ExitCode == 1 {
			t.Errorf("infrena %v --var-file overrides.yml failed:\n%s", cmd, r.combined())
		}
		if strings.Contains(r.combined(), "undefined variable") {
			t.Errorf("infrena %v did not consult --var-file:\n%s", cmd, r.combined())
		}
	}
	...
	for _, cmd := range [][]string{
		...
		r := run(t, dir, cmd...)
		if r.ExitCode != 1 {
			t.Errorf("infrena %v with no value for ${var.cidr}: exit = %d, want 1\n%s", cmd, r.ExitCode, r.combined())
		}
...
//
// SIX environments, not two. An implementation that iterates a Go map without
// sorting hits the sorted order by chance once in 720 runs with six keys, but
// roughly 88% of the time with two — which is not a test.
...
//
// destroy and refresh accept --var and --var-file because a provider instance
// configured `cloud: ${var.cloud_file}` would otherwise be refreshable and
// destroyable by nothing. provider_variables_test.go covers what the flags reach.
//
// This pins the constraint that makes accepting them safe: these commands work
// from RECORDED STATE, so a --var naming a RESOURCE's variable must not appear
// to change what they do. If it could, the user would be editing a teardown with
// a flag, and the thing destroyed would not be the thing recorded.
func TestDestroyAndRefreshAcceptVariableFlagsWithoutChangingWhatStateSays(t *testing.T) {
	...
	// cidr is a RESOURCE variable, not a provider one. Overriding it must make no
	// difference to either command: what refresh reads and what destroy tears down
	// both come from state.
	for _, args := range [][]string{
		...
		r := run(t, dir, args...)
		if r.ExitCode != 0 {
			t.Errorf("infrena %v exit = %d, want 0\n%s", args, r.ExitCode, r.combined())
		}
...
// TestPrecedenceChainEveryRungWins.
//
// Five variables, each won by a DIFFERENT rung of the precedence chain, all five
...
//
// The module-defaults rung is deliberately absent; m5_modules_test.go covers it.
...
`)
	...
	for _, tc := range []struct{ resource, want, scopeClause string }{
		...
		// d is supplied by --var-file, so its annotation names the file path
		// exactly as typed. "--var" is reserved for the literal flag.
	...
	// The whole rendered shape, asserted literally once.
...
// TestVarFileStackAppliesInFlagOrder: multiple --var-file flags apply in flag
// order, so a later file wins over an earlier one for a key both set.
...
// TestMissingVarFileIsReportedNotIgnored: a --var-file that cannot be read is
...
// typedProject declares a typed variable in infrena.yml's `variables:` block,
// referenced by an attribute whose kind must match.
...
// TestTypedVariablesAreValidatedDuringValidate asserts on `validate`
// specifically, not on plan: a type or bound violation must be reachable from
// the command whose job is finding what is wrong with a file.
func TestTypedVariablesAreValidatedDuringValidate(t *testing.T) {
	...
	requireContains(t, over.combined(), "100") // the bound it violated
...
func TestATypedVariableKeepsItsTypeThroughTheChain(t *testing.T) {
	...
	// The declared default rung still renders as an integer and still carries an
	// annotation: a declared default is never SourceExplicit, so it is never bare.
...
// TestASensitiveAttributeIsRedactedWhicheverLayerSuppliedIt: a sensitive value
// stays redacted regardless of which precedence rung supplied it, while the
// provenance annotation — metadata, not data — still shows.
...
// TestApplySuccessStillShowsAVarFileWarning. Unlike plan, apply has no later
// diagnostics pass to carry a warning through, so it renders diagnostics
// unconditionally — and nothing else reaches that render call.
//
// The warning is the reserved block-name check: a --var-file whose top-level key
// matches one of infrena.yml's own blocks is almost certainly a mistake, but only
// a warning, since a variable may legitimately be named that. So apply must still
// succeed while printing it.
...
resources:
  net:
    ...
    cidr: ${var.cidr}
`)
	// "resources" here names a VARIABLE inside the flat var-file mapping, and
	// triggers the warning precisely because a var-file cannot declare a
	// configuration block at all. Nothing else in the fixture uses it, so it
	// cannot itself affect whether apply succeeds.
	...
	// A fragment unique to this diagnostic.
...
//
// `validate` is the command run to find out what is wrong with a file, so an
// unset variable must not halt the compile before the later stages run. If it
// does, an independent mistake further down stays invisible until the variable
// is supplied and the command run again, and a user migrating a project pays a
...
// the schema declares no reference. Neither causes the other.
//
