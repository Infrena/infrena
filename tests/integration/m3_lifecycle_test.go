...
// This file covers the two lifecycle guards END TO END: from the YAML a user
// writes, through apply, into state, and back out at destroy time.
//
// Every test here seeds state by RUNNING APPLY. That is the whole point. A
// fixture that hand-builds a ResourceState with Lifecycle already set proves
// only that enforcement works IF state says so — never that state ever says so.
// Both guards can be entirely inert (the executor recording an empty Lifecycle
// on every create) while such tests pass, because destroy reads lifecycle from
// state and the resource has left configuration by then.
//
// So nothing here may call writeM2State, state.New, or write
// .infrena/state/*.json. State is only ever read.
...
// TestApplyRecordsPreventDestroyAndDestroyRefuses checks both links in the
// chain: that apply writes the configured guard into state, and that destroy
// then refuses. Drop the state assertion and the destroy is the only thing left
// standing between a guarded resource and deletion at the provider.
...
// TestApplyRecordsRetainAndDestroyForgets is the same chain for the other flag.
// retain removes a resource from management without deleting it, so the
// assertion that matters is the one about the provider.
...
// TestAddingLifecycleToAnExistingResourceReachesState covers the case every real
// user hits: the resource already exists and the guard is added afterwards.
// Nothing about its attributes changes, so a planner that diffs attributes alone
// reports "No changes", state is never rewritten, and the guard the user just
// wrote down is inert.
func TestAddingLifecycleToAnExistingResourceReachesState(t *testing.T) {
	...
	// Converged: the same configuration now proposes nothing.
...
// other flag, kept separate because retain's proof is different in kind:
// prevent_destroy is proven by a destroy that REFUSES, retain by one that
// succeeds while the resource SURVIVES at the provider.
//
// It is also the only test that reaches the retain half of lifecycleReasons: a
// create records retain without ever diffing it.
...
// TestRefreshDoesNotEraseAGuardFromState. `infrena refresh` persists whatever
// Provider.Read returns, so a Read that fails to carry Lifecycle forward does
// not leave state stale — it ERASES the guard, and the next destroy finds
// nothing to refuse. pkg/provider's Read contract requires the carry-forward
// for this reason, and this is what measures it end to end.
...
// TestPlanForConfigurationWithoutLifecycleCarriesNoLifecycle. A configuration
// that sets no lifecycle must not grow a lifecycle key in the plan artifact,
// which is what makes `omitzero` on operationWire.Lifecycle load-bearing rather
// than cosmetic.
