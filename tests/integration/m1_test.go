...
func TestValidateFailsWithoutProjectFile(t *testing.T) {
	...
	if res.ExitCode == 0 {
		t.Fatal("expected failure when infrena.yml is absent")
	}
	requireContains(t, res.combined(), "infrena.yml")
}
...
// writeState seeds a state file directly, so a test can exercise the state
// commands without running an apply first.
func writeState(t *testing.T, dir, environment string) {
	...
	doc := map[string]any{
		...
		// Only a test about migration itself should hard-code a version.
