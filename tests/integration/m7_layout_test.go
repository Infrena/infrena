...
// The conventional directory layout, through the built binary.
//
// Unit tests cover each piece — config.Load walks the directories, the compiler
// resolves a directory's variables and hands each resource the right scope. This
// file exists because the layout's failures live in the JOINS: a walk that loads
// files and decodes them into nothing leaves `validate` cheerfully reporting an
// empty resource set as valid.
...
// TestTheConventionalLayoutPlansAndApplies: four conventional directories at
// once, a reference ACROSS two resource directories, a scoped variable, and the
// whole thing reconciling.
//
...
`,
	...
	// Convergence over the whole layout: a second plan proposes nothing.
...
// TestTheDirectoryFormAndTheSingleFileFormAgree: the layout is ORGANISATION,
// not semantics. Two projects with the same content laid out differently must
// plan identically, and any difference between them is a bug in the walk.
//
// Scoped variables are deliberately absent from this fixture. They are the one
// part of the layout that is not merely organisation — resources/<dir>/vars has
// no single-file spelling at all — so including them would make the two sides
...
// TestATemplatesDirectoryIsReservedAndUnread. The name is reserved but not yet
// implemented, so a project that has one must still validate.
//
...
// scoped resources/<dir>/templates/ by the resources walk.
//
