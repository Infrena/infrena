...
// order deterministic, Go randomises map iteration, and removing the sort fails
// nothing else in the suite: a sort nothing depends on looks exactly like a sort
// nothing would miss.
//
// FIXTURE SIZE is what hides it. The other tests exercising `state show` print
// resources with two or three attributes, where a randomised order is often the
// sorted one by luck. fake.database has six, so landing sorted by chance is 1 in
// 720 per run and ten consecutive such runs are not worth worrying about.
//
