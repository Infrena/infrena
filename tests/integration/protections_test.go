...
// TestRequireApprovalRefusesAutoApprove. A protection a flag can switch off is
// not one, because the flag is a line in the pipeline that was going to run
// anyway.
//
// Exit 77 rather than 1: 77 is reserved for "changes need an approval this run
// cannot obtain", so a pipeline that already branches on it needs no new number.
...
// TestRequireApprovalIsNotAVariable. Every unrecognised key in an
// `environments:` block becomes a VARIABLE override, so a `require_approval`
// that fell through to that default would declare a variable of that name and
// protect nothing at all — silently.
