...
//
// Refusing a selection whose NAME is already in state is the collision a user
// expects to hit. The PROVIDER ID must be checked too, or importing the same
// real resource under a different name succeeds and state ends up with two
// addresses managing one piece of infrastructure.
//
// That is data loss, not untidiness: the new address declares nothing in
// configuration, so it is scheduled for destruction, and destroying it calls
// Delete on the provider ID the OTHER address still manages and configuration
// still declares. The next `apply` deletes live, declared infrastructure and
// reports success. On AWS that is a VPC.
...
`)
	...
	cloud, err := os.ReadFile(filepath.Join(dir, ".infrena", "fake-cloud.json"))
	if err != nil {
...
resources: {}
`)
	if err := os.MkdirAll(filepath.Join(dir, ".infrena"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	for _, instance := range []string{"main", "acct2"} {
		writeFile(t, filepath.Join(dir, ".infrena", "fake-cloud-"+instance+".json"), oneNetwork)
	}
