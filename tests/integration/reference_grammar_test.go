...
// TestVarPathsAndResourceAttributePathsPlanEndToEnd exercises the reference
// grammar (${var.x}, and a path after either a variable or a resource
// attribute) through a real `plan` rather than the parser's unit tests.
//
// Three shapes in one project, because each is a different consumer of the same
// path machinery and a fixture covering only one could pass while another
// regressed.
