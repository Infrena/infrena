...
//
// §38 makes a saved plan the way CI applies to a protected environment, and
// that only works if somebody reads the plan first. Until this, the only ways
// to read one were `apply --plan` answered with "no", or NDJSON by hand —
// asking the reviewer the protection depends on to invoke the APPLY command in
// order to decide whether to apply.
//
// The directory here holds the artifact and nothing else: no infra.yml, no
...
// needed the project would have put the review behind the same setup the
// pipeline has.
