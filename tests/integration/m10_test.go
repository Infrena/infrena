...
// Maps through the built binary: values that interpolate, `merge()` with an
// inline literal, and sensitivity per leaf at the only place a user meets it.
...
// TestAMapWhoseValuesInterpolateDiffersPerEnvironment.
//
...
func TestAMapWhoseValuesInterpolateDiffersPerEnvironment(t *testing.T) {
	...
	for env, line := range got {
		...
		// And nothing reached the plan as raw text.
...
// TestMergeCombinesAVariableMapWithAnInlineLiteral, in the documented spelling:
// quoted, because YAML rejects it otherwise.
...
// TestASecretInsideAMapIsRedactedAndNeverExported pins that sensitivity is per
// leaf: the secret leaf is redacted and the rest of the map stays readable.
//
...
// value — so a compiler-level version of this test proves nothing.
...
`)
	...
	// And export, which carries the same sensitivity rule.
...
// must reach the EXECUTOR, not only the plan: a missing edge here is a clean
// plan and an apply that fails partway.
...
// TestAnUnquotedLiteralSaysToQuoteIt. YAML rejects the shape before this
...
func TestTheShopExampleUsesAnInterpolatedTagMap(t *testing.T) {
	...
	if !strings.Contains(string(body), "tags:") {
		t.Error("the example does not demonstrate an interpolated tag map, which is the " +
			"whole user-visible feature it exists to show")
	}
}
