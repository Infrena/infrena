...
Removing an environment from `infrena.yml` does not silently orphan it. Infrena proposes the teardown and shows you exactly what it would destroy before you agree.
...
│       └── templates/  # templates scoped to just this directory
├── vars/               # project-wide values, one file per environment
├── modules/            # reusable components
├── templates/          # documents rendered into attributes
├── secrets/            # per-environment encrypted vaults
└── discovered/         # generated by import, reviewed by you
```

Conventional directory names are read automatically and nothing has to list them; `resources/` and `vars/` are globbed recursively. A directory scopes variables and templates but never names, so moving a file between directories renames nothing. Declaring the same name twice is an error that points at both files rather than silently keeping one.
...
State records what Infrena manages. By default it is a file per environment under `.infrena/`, locked while a run holds it, and that is the whole setup — a project that never says otherwise never has to think about this.
...
**Credentials are never configuration.** `infrena.yml` is committed, so there is deliberately nowhere in the `backend:` block to put a secret — writing one there is an error that points at `profile:` instead. Accepting it would have worked perfectly and put a long-lived key in a repository.
...
```bash
# The newest release, so this block does not go stale.
VERSION=$(curl -fsSL https://api.github.com/repos/infrena/infrena/releases/latest \
...
From there, [docs/](docs/) has the detail: the [configuration
language](docs/configuration.md), [variables and environments](docs/environments.md),
[modules](docs/modules.md), [templates](docs/templates.md), [secrets and the
vault](docs/secrets.md), [CI](docs/ci.md) and [state backends](docs/state-backends.md).
There is a worked project in [`examples/shop`](examples/shop).
...
[Contributor License Agreement](CLA.md) and why an open core project asks for one.
